...
These are the canonical build steps derived from `dkv_native/CMakeLists.txt` and
`ACTIVE_RUNTIME/native_core/dkv_core/setup.py`. They contain **no absolute/author-specific paths** —
...
> `dkv-fused-op`, the DKV fused-attention ggml/Metal kernels) exists
...
> `dkv_native/third_party/dkv-fused-op.bundle` (exact SHAs preserved).
> Restore them with:
>
> ```bash
> git submodule init
> git -C dkv_native/third_party/llama.cpp fetch origin d2462f8f7ac6d80070a587ffebf6cd73730f4280 || \
>     git submodule update --init dkv_native/third_party/llama.cpp || true
> git -C dkv_native/third_party/llama.cpp fetch \
>     "$(git rev-parse --show-toplevel)/dkv_native/third_party/dkv-fused-op.bundle" \
>     dkv-fused-op:dkv-fused-op
> git submodule update dkv_native/third_party/llama.cpp
> ```
>
> Long-term fix (TODO): push `dkv-fused-op` to a fork of llama.cpp under
> your GitHub account and point `.gitmodules` at the fork. Regenerate the
> bundle after any new submodule commits:
> `git -C dkv_native/third_party/llama.cpp bundle create ../dkv-fused-op.bundle $(git -C dkv_native/third_party/llama.cpp merge-base origin/master dkv-fused-op)..dkv-fused-op`
...
the path). On macOS `DKVHFWrapper` auto-selects the MLX backend; on CUDA it uses the PyTorch path.

---

## 3. `dkv_core` CPython extension (used by the PyTorch/MPS active path)

`setup.py` is platform-aware: on macOS it builds a `CppExtension` (Accelerate, no CUDA) and
auto-generates the embedded Metal library (`xcrun metal` → `.metallib` → `dkv_metallib.hpp`); on
...
This produces `dkv_core.cpython-<ver>-<platform>.so` **for your interpreter/arch** — it is not
portable, which is exactly why it must not be committed (see §6).

---

## 4. `dkv_native` C++ engine (GGUF / llama.cpp)
...
The Metal shader path is now portable (fixed 2026-07-02): `runtime/dkv_attention.mm` resolves
`dkv_decode.metal` via (0) the `DKV_METAL_DIR` env var, (1) executable-relative, (2) CWD-relative,
(3) a CMake-injected `DKV_METAL_SOURCE_DIR` pointing at *this build's* source tree. No home-dir path.

---

## 5. Models

GGUF weights are **not** in git (they're gitignored). Place them under `dkv_native/`, e.g.
`qwen2.5-0.5b-instruct.gguf`, `qwen2.5-1.5b-instruct-q4_k_m.gguf`. Override locations with:

```bash
export DKV_MODEL_PATH=/path/to/model.gguf
export DKV_BINARY_PATH=/path/to/dkv_native
```

The native serving gateway (`dkv_native/serving/openai_compatible_api_gateway.py`) now defaults these
...
```bash
git rm -r --cached \
  ACTIVE_RUNTIME/native_core/dkv_core/dkv_core.cpython-314-darwin.so \
  ACTIVE_RUNTIME/native_core/dkv_core/build \
  'dkv_native/native_core/**/__pycache__'
...
dkv_venv/bin/python -m pytest ACTIVE_RUNTIME/tests/test_dkv_kernel_parity.py

# Native — needle recall:
bash dkv_native/tests/test_niah_native.sh
```
