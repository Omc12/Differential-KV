...
def run_throughput_eval(batch_sizes, context_lengths, output_path):
    ...
    model_id = "Qwen/Qwen2-1.5B" # Representative model
    ...
    for bs in batch_sizes:
        ...
        for ctx_raw in context_lengths:
            ...
            # Use LCG as the primary mode for throughput benchmarking
            mode = "lcg"
            ...
            with torch.no_grad():
                ...
                past_kv_recon, _ = engine.compress_kv(past_kv, mode)
                
                # Measure 5 steps
                latencies = []
                curr_ids = torch.randint(0, 1000, (bs, 1), device=model.device)
                curr_kv = past_kv_recon
                ...
                for _ in range(5):
                    ...
                    curr_ids = outputs.logits[:, -1, :].argmax(dim=-1, keepdim=True)
                    curr_kv = outputs.past_key_values
                
                avg_latency = sum(latencies) / len(latencies)
                throughput = (bs * 1000.0) / avg_latency if avg_latency > 0 else 0
                ...
                print(f"    Latency: {avg_latency:.2f} ms | Throughput: {throughput:.2f} tok/s")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output", type=str, default="phase20/results/throughput_metrics.json")
