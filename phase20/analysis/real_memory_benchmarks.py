...
def measure_memory_metrics(models, modes, output_path):
    ...
    for model_id in models:
        ...
        # Test with a fixed context size
        ctx_len = 8192
        input_ids = torch.randint(0, 1000, (1, ctx_len), device=model.device)
        
        for mode in modes:
            print(f"  Measuring Mode: {mode}")
            ...
            with torch.no_grad():
                ...
                # 2. Compress (Measure this)
                comp_start = time.time()
                past_kv_recon, stats = engine.compress_kv(past_kv, mode)
                comp_end = time.time()
                
                # 3. Step (Measure latency)
                step_start = time.time()
                outputs = model(input_ids[:, -1:], past_key_values=past_kv_recon, use_cache=True)
                torch.cuda.synchronize()
                step_end = time.time()
            
            peak_vram = torch.cuda.max_memory_allocated() / (1024**2) # MB
            comp_latency = (comp_end - comp_start) * 1000 # ms
            step_latency = (step_end - step_start) * 1000 # ms
            
            results[model_id][mode] = {
                "peak_vram_mb": peak_vram,
                "compression_latency_ms": comp_latency,
                "step_latency_ms": step_latency,
                "throughput_tok_per_sec": 1000.0 / step_latency if step_latency > 0 else 0,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--models", nargs="+", default=["qwen2-1.5b", "llama3-8b", "mistral-7b"])
    parser.add_argument("--modes", nargs="+", default=["fp16", "int8", "rank8", "sam", "actr", "lcg"])
    parser.add_argument("--output", type=str, default="phase20/results/memory_metrics.json")
