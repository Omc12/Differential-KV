// diffkv_native/include/diagnostics.hpp
...
inline VramStats log_vram(const std::string& label = "") {
    ...
    std::fprintf(stderr,
        "[DiffKV diagnostics] %s%smemory  allocated=%.3f GB  reserved=%.3f GB"
...
// Full introspection requires access to DiffKVBlockStateTable; the pointer
...
inline void log_block_states(void* kv_manager_ptr, const std::string& session_id) {
    if (!kv_manager_ptr) {
        std::fprintf(stderr,
            "[DiffKV diagnostics] log_block_states: session='%s'"
...
public:
    ...
    void print(const std::string& label = "") const {
        std::fprintf(stderr,
            "[DiffKV diagnostics] TpsCounter %s%s steps=%d  rate=%.2f steps/s\n",
            label.empty() ? "" : label.c_str(),
            label.empty() ? "" : ":",
            steps, rate());
    }
...
inline void log_pool_stats(const std::string& label = "") {
    std::fprintf(stderr,
        "[DiffKV diagnostics] log_pool_stats%s%s — pool stats unavailable (stub).\n",
        label.empty() ? "" : "  label=",
        label.empty() ? "" : label.c_str());
}
...
#ifdef __APPLE__
    std::fprintf(stderr,
        "[DiffKV diagnostics] enable_cuda_sync_debug: no-op on macOS (no CUDA).\n");
#else
    // On CUDA builds, set CUDA_LAUNCH_BLOCKING=1 via environment or
    // cudaDeviceSetFlags(cudaDeviceScheduleBlockingSync) here.
    std::fprintf(stderr,
        "[DiffKV diagnostics] enable_cuda_sync_debug: stub — implement CUDA path.\n");
#endif
}
...
#ifdef __APPLE__
    std::fprintf(stderr,
        "[DiffKV diagnostics] disable_cuda_sync_debug: no-op on macOS (no CUDA).\n");
#else
    std::fprintf(stderr,
        "[DiffKV diagnostics] disable_cuda_sync_debug: stub — implement CUDA path.\n");
#endif
}
