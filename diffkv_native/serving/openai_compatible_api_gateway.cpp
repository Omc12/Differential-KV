...
bool DiffKVGateway::is_ephemeral_request(const std::vector<ChatMessage>& messages) {
...
std::string DiffKVGateway::optimize_ephemeral_prompt(const std::string& content) {
    ...
    for (const auto& marker : markers) {
        ...
        if (idx != std::string::npos) {
            ...
            std::cout << "[DiffKV Gateway] Optimized long title/summary prompt by truncating embedded chat history." << std::endl;
            return prefix + history;
        }
    }
    
    std::string truncated = content.substr(0, 1500) + "\n... [TRUNCATED FOR SPEED] ...\n" + content.substr(content.size() - 1500);
    std::cout << "[DiffKV Gateway] Optimized long title/summary prompt by middle-truncation." << std::endl;
    return truncated;
}

std::string DiffKVGateway::match_session_by_history(const std::vector<ChatMessage>& incoming) {
    ...
    for (auto const& [sid, session] : active_sessions) {
        ...
        if (session->history.size() == prefix_history.size()) {
            ...
            if (match) {
                std::cout << "[DiffKV Gateway] Dynamically matched message history prefix to active session: " << sid << std::endl;
                return sid;
            }
        }
    }
        ...
        for (auto const& [sid, session] : active_sessions) {
            ...
            if (found_stored) {
                ...
                if (p_last.size() > 150 && h_last == p_last) {
                    std::cout << "[DiffKV Gateway] Dynamically matched session " << sid << " using fallback last assistant message content match." << std::endl;
                    return sid;
                }
            }
        }
...
std::string DiffKVGateway::handle_chat_completion(const ChatCompletionRequest& request) {
...
void DiffKVGateway::handle_chat_completion_stream(
...
std::string DiffKVGateway::create_session() {
    if (session_manager_) {
...
void DiffKVGateway::delete_session(const std::string& session_id) {
    if (session_manager_) {
        session_manager_->delete_session(session_id);
    }
    if (batch_engine_) {
...
std::string DiffKVGateway::list_models() {
    std::string model_id = "diffkv-serving";
...
std::string DiffKVGateway::get_runtime_info() {
    ...
    std::string model_name = "diffkv-serving";
...
std::string DiffKVGateway::get_session_srl_info(const std::string& session_id) {
    if (!session_manager_) {
...
std::string DiffKVGateway::health_check() {
    return "{\"status\": \"ok\"}";
}

std::string DiffKVGateway::format_non_stream_json(
...
std::string DiffKVGateway::format_stream_chunk_sse(
    const std::string& request_id,
    long long created_time,
    const std::string& model,
