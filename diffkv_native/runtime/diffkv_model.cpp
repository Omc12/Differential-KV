...
bool DiffKVModel::load_from_file(const std::string & filename, ggml_backend_t backend) {
    std::cerr << "[DiffKV Model] Loading model from GGUF file: " << filename << " ..." << std::endl;
    ...
    if (!model_buffer_) {
        std::cerr << "[DiffKV Model] Error: Failed to allocate model tensors on backend buffer!" << std::endl;
        return false;
    }
    std::cerr << "[DiffKV Model] Allocated weight buffer of size " 
    ...
    if (!output_) {
        ...
        output_ = token_embd_;
        std::cerr << "[DiffKV Model] output.weight is tied with token_embd.weight" << std::endl;
    }
    ...
    std::cerr << "[DiffKV Model] Loaded successfully." << std::endl;
    return true;
}
