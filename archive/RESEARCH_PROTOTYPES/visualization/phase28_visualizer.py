...
def generate_plots():
    ...
    ax.bar(x + width/2, dkv_toks, width, label='DKV (Adaptive)', color='#2ecc71')
    ...
    dkv_vram = [1.2, 1.5, 2.1, 3.4]
    
    plt.figure(figsize=(10, 6))
    plt.plot(contexts, fp16_vram, 'o-', label='FP16 (Theoretical)', color='#e74c3c', linewidth=2)
    plt.plot(contexts, dkv_vram, 's-', label='DKV (Adaptive)', color='#2ecc71', linewidth=2)
    ...
    dkv_survival = [1.0, 0.99, 0.96, 0.94, 0.92]
    ...
    plt.plot(horizons, dkv_survival, '-', label='DKV Adaptive', color='#2ecc71', linewidth=2)
