...
def compress_lowrank(deltas: torch.Tensor, rank: int) -> LowRankDelta:
    ...
    if deltas.numel() == 0:
        return LowRankDelta(
            U=torch.zeros(n, rank, dtype=torch.float16, device=deltas.device),
            V=torch.zeros(rank, d, dtype=torch.float16, device=deltas.device),
