...
class AsyncCompressor:
    ...
    def __init__(self, compress_fn: Callable, max_queue: int = 256, num_workers: int = 2):
        """
    ...
    def submit(self, block, k: torch.Tensor, v: torch.Tensor) -> bool:
        ...
        try:
            self._queue.put_nowait((block, k_cpu, v_cpu))
    ...
    def _worker_loop(self) -> None:
        while self._running:
            ...
            block, k_cpu, v_cpu = item
            try:
                t0 = time.perf_counter()
                k_gpu = k_cpu.to(block.anchor_kv.device, non_blocking=False)
                v_gpu = v_cpu.to(block.anchor_kv.device, non_blocking=False)
                self._compress_fn(block, k_gpu, v_gpu)
