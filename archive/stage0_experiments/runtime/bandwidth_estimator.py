...
def estimate_bandwidth_scaling(
    ...
    diffkv_traffic = anchor_traffic + u_traffic + v_traffic + sparse_traffic
    ...
    intensity = recon_flops / (diffkv_traffic + 1e-9)
    
    return {
        "context": seq_len,
        "fp16_mb": fp16_traffic / (1024**2),
        "diffkv_mb": diffkv_traffic / (1024**2),
        "reduction_x": fp16_traffic / (diffkv_traffic + 1e-9),
        "intensity": intensity
    }

def find_crossover(heads, head_dim, rank, block_size):
    """Find at what context length DiffKV becomes bandwidth-positive."""
...
if __name__ == "__main__":
    ...
    for ctx in [4096, 16384, 65536, 131072]:
        ...
        print(f"Context {ctx:>6}: FP16 {s['fp16_mb']:>8.1f}MB | DiffKV {s['diffkv_mb']:>8.1f}MB | {s['reduction_x']:>5.2f}x reduction | Intensity: {s['intensity']:.2f}")
