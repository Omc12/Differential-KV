...
class ValidationRunner20_1:
    def __init__(self, model, tokenizer):
        self.model = model
        self.tokenizer = tokenizer
        self.symbolic_suite = MultidomainSymbolicSuite(tokenizer)
        self.noise_injector = NoisyContextInjector()
        self.results_dir = "results/reconstruction_20_1"
        os.makedirs(self.results_dir, exist_ok=True)
        self.log_file = os.path.join(self.results_dir, "raw_randomized_retrieval.jsonl")
    ...
    def execute_single_run(self, mode, ctx_len, domain, use_noise=False, run_id=0):
        ...
        if mode == "dense":
            ...
            # (Assuming a baseline sparse exists, here using a default budget)
        ...
        result = {
            ...
            "output": output_text,
            "expected": needle,
            "salience_overhead_ms": getattr(resolver, 'salience_overhead', None).get_avg_overhead_ms() if hasattr(resolver, 'salience_overhead') and resolver.salience_overhead else 0.0
        }
    ...
    def run_suite(self):
        # Mandatory contexts and modes
        contexts = [4096, 8192, 16384]
        domains = ["activation_code", "api_key", "json_snippet", "code_fragment", "multilingual"]
        modes = ["dense", "sparse_baseline", "dtascc_19_7", "asscim_20_1"]
...
if __name__ == "__main__":
    ...
    runner = ValidationRunner20_1(model, tokenizer)
