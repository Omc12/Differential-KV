...
Run (transformers MUST be 4.46.3):
  DKV_TOPK_BLOCKS=64 python colab/dkv_needle_diag.py --model Qwen/Qwen2.5-0.5B-Instruct
  DKV_TOPK_BLOCKS=64 python colab/dkv_needle_diag.py --no-hook
"""
...
# — which masked the deep-needle bug. Set DKV_SPARSE_BIAS=0 to A/B the combined
...
def main():
    ...
    def _rbr(Q, pool, block_indices, anchor_indices, scale, cos=None, sin=None, srl_state=None):
        ...
        if seen_r["n"] < 1 and NEEDLE_ANCHOR["v"] is not None:
            try:
                topk = int(os.environ.get("DKV_TOPK_BLOCKS", "16"))
    ...
    w = DKVHFWrapper(args.model, config={"rank": 32}, device=device)
    ...
    print(f"[DIAG] no_hook={args.no_hook} max_new={args.max_new} plen={ptoks} "
          f"TOPK={os.environ.get('DKV_TOPK_BLOCKS','default16')}", flush=True)
