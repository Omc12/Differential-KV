...
CELLS = [
    ...
    code(
        ...
        "!python colab/run_a100_paper_experiments.py \\",
    ...
    code(
        ...
        "    plt.tight_layout(); plt.savefig('colab/plot_frontier.png', dpi=300, bbox_inches='tight'); plt.show()",
        "",
        ...
        "    plt.tight_layout(); plt.savefig('colab/plot_quality_vs_context.png', dpi=300, bbox_inches='tight'); plt.show()",
    ),
    md(
    ...
    code(
        ...
        "    plt.tight_layout(); plt.savefig('colab/plot_pareto.png', dpi=300, bbox_inches='tight'); plt.show()",
        "else:",
    ...
    code(
        ...
        "    plt.tight_layout(); plt.savefig('colab/plot_ruler.png', dpi=300, bbox_inches='tight'); plt.show()",
        "else:",
    ...
    md("## Step 5: Camera-ready Markdown report"),
    code(
        "report_path = 'colab/diffkv_a100_paper_report.md'",
        "base = data.get('exp21_external_baselines', {})",
        "with open(report_path, 'w') as f:",
        "    f.write('# DiffKV A100 Paper Report\\n\\n## Baselines @16K NIAH (consistent metric)\\n\\n')",
        "    f.write('| Method | KV footprint (GB) | Compression | Recall % | Decode TPS | Realized? |\\n')",
        "    f.write('|---|---|---|---|---|---|\\n')",
        "    for name, r in base.items():",
        "        if not isinstance(r, dict) or r.get('status') != 'success':",
        "            continue",
        "        f.write('| %s | %.3f | %.2fx | %.0f | %.1f | %s |\\n' % (",
