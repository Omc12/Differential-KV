...
def main():
    ...
    A("3. **Coherence / needle miss (the real failure): under-budgeted sparse "
      "retrieval.** With the benchmark's `micro_block_size=256`, the selection floor "
      "(`src/main.cpp:1652`) leaves `srl_k_keep=16`, so sparse attention only ever sees "
      "≈16×256 = **4096 tokens** of compressed history — ~25% of a 16k prompt, ~12% of "
      "32k. The needle planted at 50% depth usually is not in the selected slots, so "
      "`native` hallucinated `123` at 16k instead of recalling the passcode. At 4k "
      "(fully inside the dense window) it instead echoed the instruction — a separate "
      "attention-sink/prompt-boundary issue the source itself flags "
      "(`src/main.cpp:659`: *\"loses the attention sink → echo\"*). This is a "
      "tuning-sensitive reimplementation, not yet a faithful one on this workload.\n")
