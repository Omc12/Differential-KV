...
def run_cross_arch_validation(model_id, modes, output_path):
    ...
    test_prompt = "The quick brown fox jumps over the lazy dog. Scientific discovery requires rigorous validation."
    for mode in modes:
        ...
        with torch.no_grad():
            ...
            for i in range(len(past_kv)):
                orig_k, recon_k = past_kv[i][0], past_kv_recon[i][0]
                recon_mse += torch.nn.functional.mse_loss(orig_k.float(), recon_k.float()).item()
                count += 1
            avg_mse = recon_mse / count
            results["modes"][mode] = {"avg_mse": avg_mse, "compression_ratio": stats.get("ratio", 1.0), "stability_score": 1.0 / (1.0 + avg_mse)}
