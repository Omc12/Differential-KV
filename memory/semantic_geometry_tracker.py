...
class SemanticGeometryKVManager:
    ...
    def update_importance(self, hidden_states, seq_len):
        ...
        if self.accumulated_importance is None:
            ...
            # We use max to preserve high importance from overlapping chunks
    ...
    def prune_kv(self, past_key_values, hidden_states=None):
        ...
        # Temporal Weighting: Favor recent tokens (Phase 18.9 Optimization)
        # Range 1.0 (oldest) to 2.0 (newest)
        ...
        # 2. Candidate Selection (Heavy Hitters)
        # We take more than needed to allow for neighborhood expansion
        ...
        # Protect system instructions (Sinks)
        mask[:, :self.instruction_lane_size] = True
        
        # Protect local window (Recency)
        mask[:, -self.local:] = True
        
        # Expand Heavy Hitters to include neighborhoods
        # This prevents "anchor isolation" where a symbolic token survives but its context doesn't
        ...
        # We use the mask to filter importance, then take top-k
        importance[~mask] = -1.0
