...
**Date:** 2026-07-06 (updated 2026-07-07)  •  **Auditor:** static review on Apple Silicon
(Triton falls back to PyTorch here — *no Triton/CUDA kernel was executed on CUDA*).
•  **Status:** audit + F1 correctness alignment + F3 hardening applied; F1 math Mac-verified,
**Triton compile/run + `.cu` fixes still require a GPU box.**
...
### F2 — 🟠 PERF: whole-pool `.clone()` on every decode token
- Lines 1435-1436: `anchors_K_rot = pool.anchors_K.clone(); V_K_rot = pool.V_K.clone()`,
  then only `[indices]` rows are RoPE-rotated (1438-1439).
- Cloning the entire block pool (pool_size × heads × rank × D) every token, to mutate only the
  `N` selected rows, is O(pool) bandwidth per token on the exact path we're trying to speed up.
  Rotate only the gathered `indices` rows into a small `[N, …]` scratch (reuse the workspace
  cache pattern already at 1396-1405) and pass that to the kernel.
...
### F8 — 🟠 PERF (the sync-reduction lever this whole effort is about)
- Line 411: `cudaDeviceSynchronize()` on **every** decode call. On real CUDA (unlike Mac's
  unified memory, where this was a no-op win) this is a full device barrier per token and is the
  primary sync-reduction opportunity: use per-stream sync / CUDA events, and only sync when the
  host actually reads `d_out`/`lse`. This is where the reduce-CPU/GPU-sync work that was useless
  on Apple Silicon should finally pay off. Measure before/after (checklist C5).
