...
def run_profile():
    ...
    # We use a short prompt to run quickly
    prompt = "The quick brown fox jumps over the lazy dog. Describe this event in detail."
    
    env = os.environ.copy()
    env["DIFFKV_USE_GPU"] = "1"
    env["DIFFKV_VERBOSE"] = "1"
    env["DIFFKV_TIME_DECODE"] = "1"
    env["DIFFKV_MAX_TOKENS"] = "10"
    env["DIFFKV_PRESET"] = "low"
    
    cmd = [binary_path, model_path, prompt]
    
    print("Launching C++ binary for profiling...")
