...
```
diffkv_venv/bin/python3 paper/scripts/make_facts.py      # body-text number macros -> generated/facts.tex
diffkv_venv/bin/python3 paper/scripts/make_diagrams.py   # F1-F6 architecture/dataflow + 3D memory view
diffkv_venv/bin/python3 paper/scripts/make_graphs.py     # G1-G7 measured-data graphs
diffkv_venv/bin/python3 paper/scripts/make_tables.py     # T1-T6 LaTeX tables
```
...
(preset=mid, serving_mode=balanced, rank 32, int4) with DiffKV forced on at every context:
```
...
```
Cell worker: `paper/scripts/cell_worker.py` (states the exact DiffKV config in each result JSON).
...
- **Config of record = the active CLI: mid / balanced / rank 32 / int4**, DiffKV sparse forced on
  (`DIFFKV_COMPRESSED_DECODE=1`) with decode-cache, block-sparse prefill, and adaptive bias on.
...
- **Reach**: DiffKV reaches 64k (needle intact, 4.63 GB peak); the dense full-KV baseline **OOMs**
  at 64k on the 8 GB host. That OOM boundary is the memory story's sharp end.
- **Cost**: decode throughput. DiffKV decode (~17-20 tok/s, flat) is slower than dense wherever a
