...
single representative token plus a low-rank correction---\emph{except} for a few distinctive
tokens whose detail a low-rank model discards but a later query may need verbatim. This is the
central idea of \diffkv{}: store an \emph{anchor}, a \emph{differential} low-rank term, and a
small set of \emph{exact residual tokens} per block, and never materialize the dense cache
again---not even at attention time. The low-rank term gives the bounded memory; the residuals
give back the exact recall that a pure low-rank scheme loses.
...
$256$-token block is reduced to an anchor token, a rank-$16$ joint-SVD factorization of the
per-token deltas, and the $64$ highest-reconstruction-error tokens kept exactly as residuals
(\S\ref{sec:compression}). During \emph{decode}, a single fused, \texttt{mx.compile}-d kernel
(\S\ref{sec:decode}) routes the query to the most relevant blocks by an exact residual-key
score, evaluates those blocks \emph{in low-rank coordinates}---projecting the query through each
block's $r$-dimensional basis rather than reconstructing $B{\times}d$ keys---attends their exact
residuals alongside a small uncompressed recency window, and merges the low-rank and exact
branches with a flash-style log-sum-exp. The compressed pool is pre-allocated and fixed in size,
so the KV state is bounded regardless of how long the context grows (\S\ref{sec:runtime}). An
adaptive policy decodes exactly at short context and engages the compressed kernel only when the
cache is large enough to be the binding cost.
...
\begin{enumerate}[leftmargin=1.4em,itemsep=2pt]
  \item \textbf{A differential-plus-residual block representation for KV} (anchor + rank-$r$
  joint-SVD delta over normalized per-token residuals + the top-error tokens kept exact) that
  reduces KV state by $\approx\!2.85\times$ per block at fp16 \emph{while preserving verbatim
  recall of distinctive tokens}, with a closed-form storage budget (\S\ref{sec:compression}).
  ...
  boundary, serves decode entirely from a fixed-size compressed pool plus a recency window, and
  adapts the decode path to context length so short requests never regress (\S\ref{sec:runtime}).
  \item \textbf{A fused, routed decode-attention kernel} that selects the relevant blocks by an
  exact residual-key score, computes their attention from the compressed factors without
  decompression, attends the exact residuals and recency window, and merges the two branches
  stably---so decode cost scales with the selection size, not context (\S\ref{sec:decode}).
  ...
  benefit, the throughput cost, and the residual-budget/recall trade-off, and is structured so a
  CUDA/Triton fused-decode path can be slotted in later (\S\ref{sec:eval}--\S\ref{sec:analysis}).
\end{enumerate}
