...
fill counter; and a compressed pool of $M{=}256$ pre-allocated slots
$\{U,V_K,V_V,\anchork,\anchorv,s,\ell\}$. Pre-allocation matters: the pool occupies its full
footprint from the start, so memory does not fragment or spike as blocks are added, and the
store's size is a \emph{constant} of the configuration---$M\bs{=}65{,}536$ compressed tokens
plus a $768$-token window---not a function of $N$. This is exactly why the runtime can address
a $64$k context: $65{,}536+768 \ge 65{,}615$, the largest prompt we test.
...
\State \textbf{drop native prefill cache} \Comment{decode reads the DiffKV store only}
\end{algorithmic}
\end{algorithm}

\subsection{The prefill$\to$decode boundary}
At the first decode step the runtime flushes pending lazy work, calls
\texttt{mx.clear\_cache()} and \texttt{gc.collect()} to return the peak GQA-expanded prefill
activations to the OS, and---when compressed decode is enabled---\emph{drops the native
prefill cache entirely}. From this point decode is served purely from the compressed pool and
the recency window. This boundary is what makes decode-time memory reflect the compressed
store rather than a retained full-context cache; it is also why, as \S\ref{sec:eval} shows,
the global allocator peak (set during prefill) and the steady-state decode footprint differ.
