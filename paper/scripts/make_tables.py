...
def t3_main():
    ...
    L = [r"\begin{tabular}{l" + "r" * len(ctx) + "}", r"\toprule",
         ...
         row(r"\quad MLX peak (GB)", "active", "mx_peak_gb", "%.2f"),
         row(r"\quad Needle recovered", "active", "needle_found"),
         r"\midrule",
         r"\multicolumn{%d}{l}{\textit{Dense baseline (mlx\_lm, full KV, same weights)}}\\" % (len(ctx) + 1),
         row(r"\quad Prefill (s)", "dense", "prefill_s"),
         row(r"\quad Decode (tok/s)", "dense", "decode_tps"),
         row(r"\quad MLX peak (GB)", "dense", "mx_peak_gb", "%.2f"),
