"""
Phase 1 — DKV Kernel Parity Test
====================================
Validates that `compute_decode_attention_static` (the real DKV sparse decode kernel)
...
Run:
    cd ACTIVE_RUNTIME
    python tests/test_dkv_kernel_parity.py
"""
...
def build_dkv_store_from_kv(
    ...
    """Ingest a full K/V sequence into the DKV manager token-by-token."""
    S = K.shape[2]
    for t in range(S):
        k_t = K[:, :, t:t+1, :]
        v_t = V[:, :, t:t+1, :]
...
def test_dense_only_no_compression():
    ...
    sim = cosine_sim(out_exact, out_dkv)
...
def test_needle_in_compressed_block():
    ...
    Use a query aligned with the needle's K. DKV must preserve the signal.
    Expected cosine similarity > 0.90.
    """
    ...
    dkv_norm = float(np.linalg.norm(np.array(out_dkv.astype(mx.float32))))
    print(f"  Output norm — exact: {exact_norm:.4f}, DKV: {dkv_norm:.4f}")
    print(f"  Cosine similarity: {sim:.6f}")
...
if __name__ == "__main__":
    print("=" * 60)
    print("DKV Kernel Parity Test — Phase 1")
    print("=" * 60)
