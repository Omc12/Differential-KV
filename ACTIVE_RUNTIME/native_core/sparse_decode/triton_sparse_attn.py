...
def native_triton_sparse_attn_decode(
    ...
    if N > 0:
        ...
        # Entirely dense fallback — SDPA over dense_blocks + active window
        bsz, H_q, q_len, D = q.shape
        inv_scale = 1.0 / math.sqrt(D)
        KV_GRP = num_key_value_groups

        def _repeat_kv(t, n_rep):
            """[1, H_kv, S, D] -> [1, H_q, S, D]"""
            if n_rep == 1:
                return t
            b, h, s, d = t.shape
        ...
        for blk in (dense_blocks or []):
            # Prepend anchor first, then active tokens to preserve chronological order
            k_parts.append(blk.anchor_kv[:, 0].unsqueeze(2))
        ...
        k_rep = _repeat_kv(full_k, KV_GRP)
        v_rep = _repeat_kv(full_v, KV_GRP)
