...
def lowrank_recon_kernel(
    ...
    # Scale the low-rank delta before adding anchor!
    if scale != 1.0:
        acc *= scale
...
class TritonDiffKV:
    ...
    @classmethod
    def _get_recon_buffer(cls, n_tokens, feat_dim, device, dtype):
        ...
        if key not in cls._recon_buffers or cls._recon_buffers[key].shape[0] < n_tokens:
            ...
            cls._recon_buffers[key] = torch.zeros((alloc_size, feat_dim), device=device, dtype=dtype)
        return cls._recon_buffers[key][:n_tokens]

    @staticmethod
    def reconstruct_lowrank(U, V, anchor, scale=1.0):
        # Use buffer pool to avoid aten::empty memory allocation churn.
        # IMPORTANT: We MUST .clone() the result before returning.
        # _get_recon_buffer() returns a *view* into a class-level shared buffer.
        # If callers store this view (e.g., in ReconstructionCache), the next call
        # to _get_recon_buffer() for any block will overwrite the buffer in-place,
        # silently corrupting the previously cached tensor. Cloning gives callers
        # their own independent copy that is safe to cache indefinitely.
        out_buf = TritonDiffKV._get_recon_buffer(U.shape[0], V.shape[1], U.device, U.dtype)
        # Fallback to PyTorch if Triton fails or is not available
        try:
            out = triton_fused_reconstruct(U, V, anchor, out=out_buf, scale=scale)
            return out.clone()  # Clone to decouple from the shared reuse buffer
        except Exception as e:
            print(f"Triton failed, falling back to PyTorch: {e}")
            # PyTorch fallback already allocates a fresh tensor — no clone needed
