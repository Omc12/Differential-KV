...
Tests the diffkv_core Python bindings (requires compiled extension).
...
try:
    import diffkv_core
    NATIVE = True
    print("diffkv_core native extension loaded.")
except ImportError:
    NATIVE = False
    print("[WARN] diffkv_core not compiled — validating Python integration contracts.")
...
if NATIVE:
    table = diffkv_core.DiffKVBlockStateTable()
    ...
    def stress_worker(thread_id):
        for _ in range(NUM_OPS // NUM_THREADS):
            ...
            try:
                ...
                if table.get(bid) == diffkv_core.BlockState.DenseResident:
                    table.transition(bid, diffkv_core.BlockState.DenseResident, diffkv_core.BlockState.Compressing)
            except Exception:
...
if not NATIVE:
    print("ACTION REQUIRED: Run cmake build to compile diffkv_core extension.")
    print("  cd ACTIVE_RUNTIME/native_core/diffkv_core")
