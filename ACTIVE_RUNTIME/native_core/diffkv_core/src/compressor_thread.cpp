...
void DiffKVCompressorThread::worker_loop() {
    ...
    if (cs != CUSOLVER_STATUS_SUCCESS) {
        std::fprintf(stderr, "[DiffKV] cusolverDnCreate failed: %d — compressor thread exiting.\n", cs);
        return;
    }
...
void DiffKVCompressorThread::process_job(const CompressJob& job, cusolverDnHandle_t handle) {
