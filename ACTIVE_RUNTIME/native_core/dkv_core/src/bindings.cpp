// dkv_core/src/bindings.cpp
// PyBind11 bindings for libdkv_core.
//
// Exposes:
//   Phase 0 (existing): DKVBlockStateTable
//                       DKVCompressorThread (CUDA only)
//                       DKVPagingStream     (CUDA only)
//   Phase 1 (new):      compute_query_desc, semantic_search_topk, anchor_screen,
//                       decode_attention_aten, decode_attention_aten_lse
//   Phase 1 Mac:        DKVCompressorThreadCPU (Accelerate LAPACK, no Metal)
//
...
PYBIND11_MODULE(dkv_core, m) {
    ...
    // ── DKVBlockStateTable ────────────────────────────────────────────────
    py::class_<DKVBlockStateTable>(m, "DKVBlockStateTable")
        .def(py::init<>())
        .def("transition", &DKVBlockStateTable::transition,
             py::arg("block_id"), py::arg("expected"), py::arg("desired"),
             "CAS-based atomic state transition. Raises on illegal transition.")
        .def("force_invalidate", &DKVBlockStateTable::force_invalidate,
             py::arg("block_id"),
             "Force block to Invalid state — safe to call from any thread on disconnect.")
        .def("get", &DKVBlockStateTable::get,
             py::arg("block_id"),
             "Lock-free read of current block state.")
        .def("are_replay_safe", [](const DKVBlockStateTable& self,
    ...
    // ── DKVCompressorThread ───────────────────────────────────────────────
    py::class_<DKVCompressorThread>(m, "DKVCompressorThread")
        .def(py::init<DKVBlockStateTable&, SessionAliveCallback>(),
             py::arg("state_table"), py::arg("alive_callback"),
             "alive_callback(session_id: int) -> bool")
        .def("start", &DKVCompressorThread::start,
             "Start the native worker thread.")
        .def("stop", &DKVCompressorThread::stop,
             "Stop and join the native worker thread.")
        .def("submit", &DKVCompressorThread::submit,
             py::arg("job"),
             "Non-blocking job submission. Returns False on queue overflow — block stays DenseResident.")
        .def_property_readonly("jobs_processed",  &DKVCompressorThread::jobs_processed)
        .def_property_readonly("jobs_dropped",    &DKVCompressorThread::jobs_dropped)
        .def_property_readonly("queue_overflows", &DKVCompressorThread::queue_overflows)
        .def_property_readonly("queue_depth",     &DKVCompressorThread::queue_depth);
    ...
    // ── DKVPagingStream ───────────────────────────────────────────────────
    py::class_<DKVPagingStream>(m, "DKVPagingStream")
        .def(py::init<DKVBlockStateTable&, int>(),
             py::arg("state_table"), py::arg("cuda_device") = 0)
        .def("issue_eviction", &DKVPagingStream::issue_eviction,
             py::arg("job"),
             "Issue async D2H transfer. Returns False if block not CompressedResident.")
        .def("issue_reload", &DKVPagingStream::issue_reload,
             py::arg("job"),
             "Issue async H2D transfer. Returns False if block not CPUResident.")
        .def("poll_completions", &DKVPagingStream::poll_completions,
             "Poll pending transfers; advance state machine for completed ones. Call between batch steps.")
        .def_property_readonly("evictions_completed",  &DKVPagingStream::evictions_completed)
        .def_property_readonly("reloads_completed",    &DKVPagingStream::reloads_completed)
        .def_property_readonly("cancelled_transfers",  &DKVPagingStream::cancelled_transfers);
#endif  // DKV_CUDA
    ...
    py::class_<DKVCompressorThreadCPU>(m, "DKVCompressorThreadCPU")
        .def(py::init<DKVBlockStateTable&, CPUSessionAliveCallback>(),
             py::arg("state_table"), py::arg("alive_callback"),
             "alive_callback(session_id: int) -> bool\n"
             "Accelerate LAPACK SVD — no Metal/MPS calls, AMX-accelerated on Apple Silicon.")
        .def("start", &DKVCompressorThreadCPU::start,
             "Start the CPU compressor worker thread.")
        .def("stop", &DKVCompressorThreadCPU::stop,
             "Stop and join the worker thread.")
        .def("submit", &DKVCompressorThreadCPU::submit,
             py::arg("job"),
             "Non-blocking submission. Returns False on queue overflow.")
        .def_property_readonly("jobs_processed",  &DKVCompressorThreadCPU::jobs_processed)
        .def_property_readonly("jobs_dropped",    &DKVCompressorThreadCPU::jobs_dropped)
        .def_property_readonly("queue_overflows", &DKVCompressorThreadCPU::queue_overflows)
        .def_property_readonly("queue_depth",     &DKVCompressorThreadCPU::queue_depth);
#endif  // DKV_APPLE
...
Returns:
    ...
    m.def("fused_decode_attention_combined",
        &dkv::fused_decode_attention_combined,
        py::arg("Q"),
        py::arg("dense_k"),
        py::arg("dense_v"),
        py::arg("cos_dense"),
