...
# DKV_SPARSE_BIAS — additive bias (in nats) on the compressed/sparse half's LSE in the
...
# forced-half + DKV_DBG_LSE_SHARE). A +2.0 bias tips the blend to the paper (filler→paper
...
def _capture_policy_env() -> Tuple[bool, float]:
    ...
    DKV_RES_V_ONLY=1 — keep the JOINT error ranking, but store
    ...
    DKV_RESIDUAL_COVERAGE_FRAC=f — reserve round(f*max_residual) slots for
    ...
    """
    res_v_only = os.environ.get("DKV_RES_V_ONLY", "0") == "1"
    try:
...
def _apply_owner_capture(boost_multipliers, segment_indices, is_core, tok_strs,
    ...
    """Relational-locality capture (DKV_RESIDUAL_OWNER_CAPTURE, default ON).
    ...
    Fix: for every core segment, walk LEFT up to DKV_RESIDUAL_OWNER_DIST
...
def _apply_relational_capture(boost_multipliers, severity, tok_strs, tok_boost,
                              dbg=None):
    """Reconstruction-COLLISION edge capture (DKV_RESIDUAL_EDGE_CAPTURE).
    ...
    **The signal (no word list, not norm-degenerate).** A connective vocabulary is
    ...
    Capture the top DKV_EDGE_MAX non-node tokens whose severity clears an
    absolute floor (DKV_EDGE_FLOOR) AND the block median + DKV_EDGE_MARGIN
    (so a collision-free block captures nothing → decode cost stays flat), boosted
    to a fixed level below the content nodes (DKV_EDGE_SCALE). Returns the count.
    `severity` is the (S_comp,) per-token, layer-meaned collision score.

    EXPERIMENTAL, DEFAULT OFF (2026-07-14). Direct instrumentation (DKV_DBG_EDGE)
    ...
    if os.environ.get("DKV_RESIDUAL_EDGE_CAPTURE", "0") != "1":
        return 0
    import math as _math
    try:
        edge_floor = float(os.environ.get("DKV_EDGE_FLOOR", "0.02"))
    except ValueError:
        edge_floor = 0.02
    try:
        edge_margin = float(os.environ.get("DKV_EDGE_MARGIN", "0.02"))
    except ValueError:
        edge_margin = 0.02
    try:
        edge_scale = float(os.environ.get("DKV_EDGE_SCALE", "0.7"))
    except ValueError:
        edge_scale = 0.7
    try:
        edge_max = int(os.environ.get("DKV_EDGE_MAX", "6"))
    except ValueError:
        edge_max = 6
...
def _apply_table_capture(boost_multipliers, tok_strs, tids, counts,
    ...
    """Structured-data capture (DKV_RESIDUAL_TABLE_CAPTURE, default ON).
    ...
    DKV_RESIDUAL_TABLE_PRIORITY (default 4). Under saturation the err×boost
...
def compress_mlx_block_batched(deltas: mx.array, rank: int, n_oversamples: int = 5, n_iter: int = 2) -> Tuple[mx.array, mx.array, mx.array]:
    ...
    seed = int(os.environ.get("DKV_SVD_SEED", "1234"))
...
def compute_decode_attention_static(
    ...
    # Correct the compressed pool's systematic LSE under-scoring (see DKV_SPARSE_BIAS
...
def _edge_propagate_relevance(relevance, anc_k, beta, max_nb):
    """Edge-aware routing (DKV_EDGE_ROUTING, default ON).
    ...
    token). β = DKV_EDGE_ROUTE_BETA (default 0.25); skipped above max_nb blocks
...
def _edge_routing_params():
    ...
    if os.environ.get("DKV_EDGE_ROUTING", "1") != "1":
        return False, 0.0, 0
    try:
        beta = float(os.environ.get("DKV_EDGE_ROUTE_BETA", "0.25"))
    except ValueError:
        beta = 0.25
    try:
        max_nb = int(os.environ.get("DKV_EDGE_ROUTE_MAXNB", "512"))
    except ValueError:
        max_nb = 512
...
def _lego_prefill_attend(
    ...
    """LEGO streaming-prefill attention (DKV_LEGO_PREFILL — see the flag note
    ...
    built entirely from the DKV session state — the raw prompt KV cache is
    never consulted, so prefill peak raw KV is O(sinks + ring + chunk), not O(T).
    The ring (last ~DKV_LEGO_RING tokens, raw and exact) is the chunk's local
    ...
    # DKV_LEGO_FP32 — SDPA dtype dial. Default now FOLLOWS THE ROW SOURCE:
    ...
    # a knife-edge; mirrors DKV_DECODE_FUSED_FP32). Explicit env overrides both.
    _fp32_env = os.environ.get("DKV_LEGO_FP32")
    _use_recon = os.environ.get("DKV_LEGO_RECON", "0") == "1"
    ...
    # DKV_LEGO_RECON (default 0 — studs-only): whether routed blocks contribute
    ...
    # =1 re-enables recon rows (pair with DKV_LEGO_BIAS to counter their
    ...
    if _use_recon:
        ...
        # DKV_LEGO_BIAS — additive score bias (nats) on the RECON block rows only
        # (not residuals, not sinks/ring), the same correction the decode paths apply
        # (see DKV_SPARSE_BIAS at module top): low-rank reconstruction
        ...
        try:
            _lego_bias = float(os.environ.get("DKV_LEGO_BIAS", "0"))
        except ValueError:
            _lego_bias = 0.0
...
class MLXKVBlockManager:
    ...
    def __init__(self, num_layers: int, heads: int, kv_heads: int, head_dim: int, rank: int, block_size: int, recency_window: int = 512):
        ...
        # Configurable via DKV_MAX_BLOCKS env var (lower = less VRAM).
        self.max_blocks = int(os.environ.get("DKV_MAX_BLOCKS", "256"))
        ...
        # on retrieval-only workloads, set DKV_MAX_RESIDUAL=64.
        # Restore default to 128 for full correctness and to prevent repetitive loops.
        # Users can override this to 32 or 64 via DKV_MAX_RESIDUAL if needed.
        self.max_residual = int(os.environ.get("DKV_MAX_RESIDUAL", "128"))
        ...
        _hq = os.environ.get("DKV_HIGH_QUALITY_ROUTING", "0").strip().lower()
        ...
        #     summary routers fail. DKV_ROUTE_RESIDUALS = R; 0 (default) means use
        ...
        self.router = os.environ.get("DKV_ROUTER", "residual").lower()
        # DKV_ROUTE_RESIDUALS = how many residuals per block the ROUTER scores when ranking
        ...
        # router. Raise DKV_ROUTE_RESIDUALS toward max_residual for 64k+ (deep mid-rank
        ...
        # To restore the decode cache speedup: DKV_DECODE_CACHE=1
        self._decode_cache = os.environ.get("DKV_DECODE_CACHE", "0") == "1"
        # DKV_DECODE_FUSED=1 (default): single-SDPA-launch per layer per token over a
        ...
        # separate follow-up (see dkv_on_the_fly_bugs.md).
        ...
        # relational 4/4. Reversible: DKV_SPARSE_PREFILL=0. Compute-only (no memory change);
        # only engages above _sp_min_ctx tokens, no-op below.
        self._sparse_prefill = os.environ.get("DKV_SPARSE_PREFILL", "1") != "0"
        ...
        self._sp_min_ctx = int(os.environ.get("DKV_SPARSE_PREFILL_MIN", "2048"))
        # Exact recency window (tokens, always attended, in addition to the current chunk).
        self._sp_window = int(os.environ.get("DKV_SPARSE_PREFILL_WINDOW", "1024"))
        # Number of leading blocks kept as always-attended attention sinks (StreamingLLM).
        self._sp_sink_blocks = int(os.environ.get("DKV_SPARSE_PREFILL_SINK_BLOCKS", "1"))
        ...
        # (routing reads raw keys), so prefill peak memory stays O(T) — and DKV holds
        ...
        # (docs/NATIVE_LEGO_PORT_PLAN.md): DKV_LEGO_PREFILL=1 for memory-
        ...
        self._lego_prefill = os.environ.get("DKV_LEGO_PREFILL", "0") == "1"
        self._lego_min_ctx = int(os.environ.get("DKV_LEGO_MIN_CTX", str(self._sp_min_ctx)))
        self._lego_kmin = int(os.environ.get("DKV_LEGO_KMIN", str(self._sp_kmin)))
        self._lego_frac = float(os.environ.get("DKV_LEGO_FRAC", str(self._sp_frac)))
        ...
        self._lego_router = os.environ.get("DKV_LEGO_ROUTER", "minmax").strip().lower()
        ...
        _ring = int(os.environ.get("DKV_LEGO_RING", "4096"))
        ...
        # store. Gated behind DKV_FACTUAL_STORE (default off) so the validated
        # sparse path is untouched until this is proven out.
        self._factual_enabled = os.environ.get("DKV_FACTUAL_STORE", "0").strip().lower() in ("1", "on", "true", "yes")
    ...
    def finalize_srl_index(self, session_id: str, cached_len: int = 0):
        ...
        try:
            ...
            if os.environ.get("DKV_TELEMETRY", "0") == "1":
                ...
                if os.environ.get("DKV_FACTUAL_DBG") == "1" and self.tokenizer is not None:
    ...
    def compress_deferred_prefill_blocks(self, session_id: str):
        ...
        if num_blocks > 0 and start_blocks + num_blocks > max_b:
            print(f"[DKV MLX] WARNING: session '{session_id}' block pool full "
        ...
        tok_boost_env = os.environ.get("DKV_RESIDUAL_TOKEN_BOOST")
        tok_boost = 8.0
        ...
        if os.environ.get("DKV_DBG_TABLE") == "1":
        ...
        if tok_boost > 1.0 and "token_ids" in session and len(session["token_ids"]) > 0:
            ...
            for block_idx in range(num_blocks):
                ...
                if len(tids) == S_comp:
                    ...
                    _dbg_edge = [] if os.environ.get("DKV_DBG_EDGE") == "1" else None
        ...
        # Floor = boosted + n_cov + margin (DKV_RESIDUAL_FLOOR_MARGIN, def 4).
        res_v_only, cov_frac = _capture_policy_env()
        if boost_rows:
            try:
                _floor_margin = int(os.environ.get("DKV_RESIDUAL_FLOOR_MARGIN", "4"))
            except ValueError:
                _floor_margin = 4
            _n_cov = 0
        ...
        if self.max_residual > 0:
            ...
            if os.environ.get("DKV_DBG_TABLE") == "1" and getattr(self, "_dbg_table_blocks", None):
    ...
    def _get_or_create_session(self, session_id: str):
        ...
        if session is None:
            ...
            init_blocks = min(self.max_blocks,
                              max(1, int(os.environ.get("DKV_INIT_BLOCKS", "16"))))
    ...
    def _execute_decode_cache(self, session, layer_idx, q, dense_k, dense_v, dense_len, scale, gpk):
        ...
        FUSED per-token path (DKV_DECODE_FUSED=1, default ON, 2026-07-10): at route time the
        ...
        Numerics notes:
          - Buffer dtype defaults to fp32 (DKV_DECODE_FUSED_FP32, see the dial comment
        ...
        DKV_DECODE_FUSED=0 restores the old concat-per-token behavior exactly."""
        nb = session["num_blocks"][layer_idx]
        ...
        if need_route:
            ...
            # DKV_DECODE_CACHE=1 "kinda right but corrupted" regression. (The int8-U
            ...
            # to the no-bias flash merge). A numeric DKV_SPARSE_BIAS is still honored as a flat
            ...
            if fused:
                ...
                # DKV_DECODE_FUSED_FP32 — buffer dtype dial (default "1" = fp32):
                ...
                _fp16_buf = os.environ.get("DKV_DECODE_FUSED_FP32", "1").strip().lower() in ("0", "off", "false")
    ...
    def execute_decode_attention(self, session_id: str, layer_idx: int, q_rot: mx.array, rope: Any, scale: float, num_key_value_groups: int) -> mx.array:
        ...
        route_once = os.environ.get("DKV_ROUTE_ONCE", "0") == "1" or getattr(self, "route_once", False)
        if layer_idx == 0:
        ...
        if all_blocks_full:
            ...
            if os.environ.get("DKV_DBG_LSE_SHARE") == "1":
        ...
        if os.environ.get("DKV_DBG_NAN") == "1":
...
def _resolve_compressed_decode(seq_len: int) -> bool:
    """Decide whether decode should use the DKV compressed sparse kernel.

    DKV_COMPRESSED_DECODE:
      "1"/"on"/"true"   → always compressed (force the sparse path). THE DEFAULT —
                          DKV's sparse decode engages from token 1 so the
      ...
      "auto"            → OPT-IN adaptive: dense below DKV_COMPRESSED_MIN_CTX
                          (default 16384), sparse at/above it. Avoids the
                          short-context regression; use when raw short-prompt
                          throughput matters more than always exercising DKV.
    """
...
class MLXQwenModel:
    ...
    def _get_or_create_prefill_cache(self, cache_key: tuple, total_tokens: int = 0):
        if cache_key not in self._prefill_caches:
            ...
            # DKV_PREFILL_CACHE_BITS: bit-width for the prompt KV cache during prefill.
            ...
            # Users can override this to 8 or 4 via DKV_PREFILL_CACHE_BITS if needed.
            _cache_bits = int(os.environ.get("DKV_PREFILL_CACHE_BITS", "16"))
    ...
    def __call__(self, input_ids: torch.Tensor, position_ids: torch.Tensor, use_cache: bool = True):
        ...
        self.manager.active_session_ids = self._dkv_session_ids
        ...
        cache_key = tuple(self._dkv_session_ids)
        ...
        if is_prefill:
            ...
            # Engaged chunks attend the DKV session state only (sinks + compressed
            ...
            # patched attention uses the DKV store instead.)
...
class MLXDKVWrapper:
    ...
    def ensure_loaded(self):
        ...
        # DKV_CACHE_LIMIT_GB=0 disables the cap.
        cache_gb = os.environ.get("DKV_CACHE_LIMIT_GB", "1")
        try:
        ...
        # DKV_CAD_MAX_STEPS tokens (the relation is stated early): a 32-token
        ...
        if preset in ("high", "quality", "max"):
            ...
            print(f"[DKV MLX] {preset} preset: Context-Aware Decoding on "
                  f"(alpha={os.environ['DKV_CAD_ALPHA']}, "
                  f"max_steps={os.environ['DKV_CAD_MAX_STEPS']})")
        ...
        if quant in ("int4", "int8") and not model_id.startswith("mlx-community/"):
            ...
            if len(parts) == 2:
                ...
                print(f"[DKV MLX] Loading quantized model: {model_id}")

        print(f"[DKV MLX] Loading model via mlx_lm: {model_id}...")
        t0 = time.time()
        model, tokenizer = mlx_load(model_id)
        print(f"[DKV MLX] Loaded model in {time.time() - t0:.2f}s")
        ...
        # (no-ops unless DKV_FACTUAL_STORE is enabled).
        ...
        self.model.keep_prefill_cache = (
            ...
            or os.environ.get("DKV_SPECULATIVE", "0") == "1"
        )
    ...
    def generate(
        ...
        if session_id in self._session_token_ids:
            ...
            if len(stored_ids) > 0 and len(stored_ids) < len(prompt_ids):
                if prompt_ids[:len(stored_ids)] == stored_ids:
                    ...
                    print(f"[DKV MLX Wrapper] Reusing {cached_len} cached tokens!")
        ...
        # runs as its own session, switched in per forward. Gated by DKV_CAD_ALPHA
        ...
        # is instead to CAP CAD to the first DKV_CAD_MAX_STEPS decode tokens
        ...
        if _cad_on:
            ...
            try:
                ...
                print(f"[DKV CAD] disabled (prior prefill failed: {_e})", flush=True)
                _cad_on = False
            finally:
                self.model._dkv_session_ids = [session_id]
        ...
        for _ in range(max_new_tokens):
            ...
            if not _loop_detected and _n_new >= 30 and _n_new % 10 == 0:
                ...
                if _exact_loop or _ratio_loop:
                    ...
                    print(
                        f"[DKV MLX] WARNING: repetition loop detected at token "
            ...
            if _loop_detected:
                if _loop_idx is None:
                    ...
                    print(
                        "[DKV MLX] WARNING: repetition loop persisted for 40 tokens "
            ...
            if _pen_val != 1.0:
                ...
                # DKV_REP_PENALTY_PROTECT_NUMERIC=0 restores.
            ...
            if srl_state is not None and os.environ.get("DKV_FACTUAL_DBG") == "1":
