...
## Integration Architecture (`vllm.attention.backends.dkv`)
...
- **Action:** OpenWebUI (or API client) starts vLLM with `--model d-ffkv-qwen2.5-0.5b-Instruct --kv-cache-dtype dkv`.
- **vLLM Core:** Detects `dkv` dtype. Instantiates `DKVAttentionBackend`.
...
- **Worker Init:** `DKVCompressorThread` and `DKVPagingStream` native C++ objects are instantiated per GPU worker.

### 2. Request Ingestion & Prefill
- **Action:** User sends a 10K token prompt.
- **vLLM Scheduler:** Allocates dense blocks.
- **Forward Pass:** `DKVAttentionImpl.forward_prefill()` delegates to standard FlashAttention/XFormers for the dense prefill phase. KV cache is written to the Dense pool.
...
- **Background SVD:** The C++ `DKVCompressorThread` executes cuSOLVER SVD and writes to the appropriate Slab pool (8, 16, or 32).
- **State Transition:** The native `DKVBlockStateTable` atomically transitions the block from `Compressing` to `CompressedResident`.
...
- **Kernel Dispatch:** `DKVAttentionImpl.forward_decode()` executes a split dispatch:
