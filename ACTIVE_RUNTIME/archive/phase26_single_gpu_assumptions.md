...
The `TritonDiffKV` and PyTorch SDPA paths expect $Q$, $K$, and $V$ (or $U$ and $V$) to reside in local SRAM/HBM. They cannot read pointers from a neighboring GPU over NVLink unless specifically mapped via Unified Memory (which is incredibly slow for unstructured access) or manually exchanged.
