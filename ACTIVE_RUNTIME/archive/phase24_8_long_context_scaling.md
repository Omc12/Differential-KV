...
| Prompt Length | Dense Baseline (Eager SDPA) | DiffKV (Eager) | DiffKV (FlashAttention SDPA) | DiffKV (Chunked Sparse Prefill) |
|---|---|---|---|---|
...
1. **Dense Baseline / DiffKV (Eager):** Both scale quadratically ($O(N^2)$) due to materializing the attention score matrix in HBM. They fail at 25K tokens.
2. **DiffKV (FlashAttention SDPA):** PyTorch's native SDPA avoids materializing the $O(N^2)$ matrix, evaluating it in SRAM instead. The memory overhead scales linearly with sequence length because the final output tensor and inputs (Q, K, V) still grow as $O(N)$. 
3. **DiffKV (Chunked Sparse Prefill):** By slicing the sequence into 512-token chunks, the active context (Q, K, V) in SRAM is strictly bounded. The activation memory overhead flattens completely to ~$O(1)$ regardless of sequence length.
