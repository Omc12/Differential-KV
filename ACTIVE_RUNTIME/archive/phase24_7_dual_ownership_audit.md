...
1. **Bytes owned by legacy dense KV cache:** 0 bytes.
   ...
   - The `dkv_attention.py` monkey-patch explicitly intercepts KV storage and routes it to `kv_manager.ingest_streaming()`, returning `None` to the HF native cache mechanisms.

2. **Bytes owned by DKV slabs:** ~340 MB (for 25K tokens, Qwen-0.5B).
