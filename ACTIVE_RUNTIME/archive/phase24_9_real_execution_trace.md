...
hf_diffkv_wrapper.py (Model Wrapper)
  ↓ (Forward pass)
Qwen2DecoderLayer.forward()
  ↓ (Monkey-patched)
make_diffkv_forward() -> diffkv_forward()
...
1. **Module Replacement Success:** Verified. `apply_diffkv_attention_patch` overrides `layer.self_attn.forward` on all 24 layers of the Qwen2 model.
