...
# Run NIAH test for C++ native runtime with Q4_K_M model.
# Sweep 4k/8k/16k × depth 0.5/0.9.
...
MODEL="${DKV_TEST_MODEL:-../qwen2.5-1.5b-instruct-q4_k_m.gguf}"
if [ ! -f "$MODEL" ]; then
...
for ctx in "${CONTEXTS[@]}"; do
    for depth in "${DEPTHS[@]}"; do
        echo "============================================================"
        echo "Testing Native Q4 NIAH: ctx=$ctx depth=$depth"
        echo "============================================================"
        prompt=$(python3 make_niah_prompt.py "$ctx" "$depth" "$NEEDLE" "$QUESTION")
        # Run C++ binary
        output=$("$BINARY" "$MODEL" "$prompt" 2>native_q4_test_stderr.log) || true
        
        # Check output
        if echo "$output" | grep -qi "OMEGA-7741-DELTA"; then
            echo "Result: PASS ✓"
            pass=$((pass+1))
            # Append success to overall log
            echo "Q4 NIAH PASS: ctx=$ctx depth=$depth" >> niah_q4_results.log
        else
            echo "Result: FAIL ✗"
            echo "Output was: $output"
            fail=$((fail+1))
            # Append fail to overall log
            echo "Q4 NIAH FAIL: ctx=$ctx depth=$depth" >> niah_q4_results.log
        fi
        echo ""
    done
done
...
echo "============================================================"
echo "Native Q4 NIAH Results: $pass PASS / $fail FAIL"
echo "============================================================"
