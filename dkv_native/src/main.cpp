...
#include "runtime/dkv_attention.hpp"
...
#include <atomic>
namespace dkv { extern std::atomic<long> g_dkv_cb_invocations; }  // diagnostic counter (dkv_attention.cpp)
namespace dkv { extern std::atomic<long> g_cpu_attn_count; extern std::atomic<long> g_metal_attn_count; }
namespace dkv { extern std::atomic<int> g_dkv_dbg_pos; extern std::atomic<int> g_dkv_dbg_block_states; }
static std::atomic<bool> g_dkv_enable_factual{true};

static bool is_native_attn_enabled() {
    const char* e = std::getenv("DKV_NATIVE_ATTN");
    if (e) {
        return (std::string(e) == "1" || std::string(e) == "true" || std::string(e) == "yes" || std::string(e) == "on");
    }
    // Default OFF: measured 2026-07-03 at 4k (DKV_PROFILE=1), the fused
...
// DKV_DECODE_CACHE=1: "decompress-and-cache" sparse decode (the MLX §BIG-WIN, ported).
...
// DKV_MLX_PARITY, if set, still overrides this per-request classification
// globally (useful for isolated benchmarking of either mode).
static bool dkv_detect_narrow_query(const std::string & prompt_text) {
...
static bool is_decode_cache_enabled() {
    ...
    // decode is unaffected. Explicit DKV_DECODE_CACHE=0 forces the old CPU-op path (for
    // baseline measurement). Matches the MLX serving default (DKV_DECODE_CACHE=1).
    const char* e = std::getenv("DKV_DECODE_CACHE");
    if (!e) return true;
    std::string v(e);
...
static int decode_cache_interval() {
    const char* e = std::getenv("DKV_DECODE_CACHE_INTERVAL");
    int n = e ? atoi(e) : 16;
    return n > 0 ? n : 16;
}
...
struct ggml_backend_owner {
    ...
    ggml_backend_owner() {
        bool use_gpu = true;
        if (const char* env_gpu = std::getenv("DKV_USE_GPU")) {
...
struct ggml_cgraph * build_prefill_graph(
    ...
    for (int l = 0; l < config.n_layer; ++l) {
        ...
        // dkv_attention.cpp decode callback applies RoPE at query time (has_rope=true).
...
struct ggml_cgraph * build_prefill_ctx_graph(
    ...
    int pos_start = 0,
    // DKV_SPARSE_PREFILL (HANDOFF §SPARSE-PREFILL): when non-empty, k_ctx/v_ctx attend only
    ...
    // cache; ring-mapped by the caller under DKV_LEGO_PREFILL.
    const std::vector<std::pair<int,int>>* sp_ranges = nullptr,
    // DKV_LEGO_PREFILL: buffer spans to write the current chunk into (the ring write may
    ...
    // DKV_LEGO_PREFILL: per-layer device tensors holding the routed FAR blocks' rows
    ...
    for (int l = 0; l < config.n_layer; ++l) {
        ...
        if (const char* env_cpg = std::getenv("DKV_PREFILL_CAST_GPU")) {
        ...
        // RoPE is re-applied at decode time by dkv_attention.cpp (has_rope=true).
...
// DEBUG (DKV_DBG_CMP): capture layer-0 native q_rope + sparse attn_out so main can diff vs
...
// DKV_BLOCK_CMP: per-layer tensor captures for full block-wise comparison across all layers.
...
// ── DKV_DECODE_CACHE materialize helper ─────────────────────────────────────────────
...
// DKV_DBG_RECON_POS), so it drops straight into flash. V is raw. The reconstruction math
    ...
    ggml_fp16_t* out_k, ggml_fp16_t* out_v)
{
    ...
    const int MR = dkv::NativeBlockPool::MAX_RESIDUAL;
    ...
    for (int si = 0; si < n_slots && blocks_done < max_blocks && written < cap_tokens; ++si) {
        ...
        if (state_table.get(slot) != dkv::BlockState::CompressedResident) continue;
...
// ── DKV_LEGO_PREFILL stage-2: single-block far emit from the compressed pool ──
...
static int lego_emit_block_from_pool(
    dkv::NativeBlockPool* pool, int slot, int F,
    ggml_fp16_t* out_k, ggml_fp16_t* out_v)
{
    const int R  = pool->get_rank();
    const int MR = dkv::NativeBlockPool::MAX_RESIDUAL;
...
// count (1 + mr_target) is guaranteed by DKV_RESIDUAL_UNIFORM=1 at compression
...
static int lego_emit_block_studs(
    dkv::NativeBlockPool* pool, int slot, int F, int mr_target,
    ggml_fp16_t* out_k, ggml_fp16_t* out_v)
{
    const int R  = pool->get_rank();
    const int MR = dkv::NativeBlockPool::MAX_RESIDUAL;
...
struct ggml_cgraph * build_decode_graph(
    ...
    struct ggml_tensor ** cache_k = nullptr,           // DKV_DECODE_CACHE: [n_layer] each [head_dim,kv_heads,cache_cap] materialized routed+window K (F16, rotated)
    struct ggml_tensor ** cache_v = nullptr,           // DKV_DECODE_CACHE: [n_layer] each [head_dim,kv_heads,cache_cap] materialized routed+window V (F16)
    struct ggml_tensor * cache_mask = nullptr,         // DKV_DECODE_CACHE: [cache_cap+1,1] F16 validity bias (0 valid / -inf pad; +1 = current token)
    int cache_cap = 0,                                 // DKV_DECODE_CACHE: capacity of the cache buffers (routed + window tokens)
    ...
    for (int l = 0; l < config.n_layer; ++l) {
        ...
        if (use_sparse) {
            ...
            if (userdata && userdata[l].srl_state) {
                factual_empty = static_cast<dkv::SessionSRLState*>(userdata[l].srl_state)->factual_store.entries.empty();
            }
            bool use_native_attn = native_attn_env && factual_empty &&
                                   userdata && userdata[l].kv_engine &&
                                   userdata[l].kv_engine->native_attn_enabled();

            if (l == 0 && std::getenv("DKV_DBG_ATTN0")) {
            ...
            if (decode_cache_on && cache_k && cache_v && cache_mask && cache_k[l]) {
                // ── DKV_DECODE_CACHE: attend [materialized routed blocks ++ dense window
                ...
                // Flash-decoding in-graph fused op (GGML_OP_DKV_ATTN)
                dkv::NativeBlockPool* pool = userdata[l].kv_engine;
                const int F_kv = head_dim * config.n_head_kv;
                struct ggml_dkv_attn_params p = {
                ...
                attn_out = ggml_dkv_attn(ctx, q_rope_flat,
                    ...
                    p);
                { const char* cl = std::getenv("DKV_DBG_CMP_LAYER"); int cmpL = cl ? atoi(cl) : 0;
                  if (l == cmpL && std::getenv("DKV_DBG_CMP")) {
                ...
                // DKV_BLOCK_CMP: capture every layer for full multi-layer comparison
                if (std::getenv("DKV_BLOCK_CMP") && l < BLOCK_CMP_MAX_LAYERS) {
...
// DKV_DENSE_CMP=1: standalone comparison of cpu_dense_attention vs ggml rope_ext + plain
...
static void run_dense_attn_cmp() {
    ...
    if (const char* e = std::getenv("DKV_DENSE_CMP_T")) T = std::stoi(e);
...
// DKV_RECON_CMP=1: standalone per-token K/V reconstruction-fidelity probe (no model load).
...
// diff the NEEDLE error to localize the native-vs-active gap. Set DKV_RECON_NORESID=1 to
// also see native's pure-lowrank (no-residual) error, apples-to-apples with active (no residuals).
static void run_recon_cmp() {
    using dkv::LowRankCompressParams;
    const int S = 256, KVH = 2, D = 128;
    const int F = KVH * D;            // 256
    int R = 16;
    if (const char* e = std::getenv("DKV_RANK")) R = std::max(1, atoi(e));
    const bool no_resid = (std::getenv("DKV_RECON_NORESID") != nullptr);
    ...
    // feed that index to recon_cmp.py via DKV_RECON_ANCHOR=L so both use the same anchor
    ...
    for (int i = 0; i < num_loops; ++i) {
        if (!dkv::compress_lowrank_block(p)) { std::cerr << "[RECON_CMP] compress failed\n"; return; }
    }
...
// DKV_ATTN_CMP=1: standalone decode-attention faithfulness probe (no model load).
...
// DKV_ATTN_CMP_ROPE=1 enables RoPE (default off isolates the value/score path from rotation);
// DKV_ATTN_CMP_EXACT=1 uses native's non-approximate attention path.
static void run_attn_cmp() {
    const int nq = 4, nkv = 2, D = 128, S = 64, desc_dim = 8;
    const int F = nkv * D;            // 256
    int rank = 16;
    if (const char* e = std::getenv("DKV_RANK")) rank = std::max(1, atoi(e));
    const bool HR = (std::getenv("DKV_ATTN_CMP_ROPE") != nullptr);
    const bool approximate = (std::getenv("DKV_ATTN_CMP_EXACT") == nullptr);
    const bool use_resid = (std::getenv("DKV_ATTN_CMP_RESID") != nullptr);
    int NB = 1;  // number of blocks (DKV_ATTN_CMP_BLOCKS): >1 probes the multi-block 8k+ bug
    if (const char* e = std::getenv("DKV_ATTN_CMP_BLOCKS")) NB = std::max(1, atoi(e));
...
int main(int argc, char ** argv) {
    ...
    if (std::getenv("DKV_DBG_POS")) dkv::g_dkv_dbg_pos.store(1);  // mirror to a global (worker-thread getenv fails)
    if (std::getenv("DKV_DBG_BLOCK_STATES")) dkv::g_dkv_dbg_block_states.store(1);  // mirror to a global
    if (const char* ef = std::getenv("DKV_ENABLE_FACTUAL")) {
        if (std::string(ef) == "0" || std::string(ef) == "false" || std::string(ef) == "off") {
            g_dkv_enable_factual.store(false);
        } else if (std::string(ef) == "1" || std::string(ef) == "true" || std::string(ef) == "on") {
            g_dkv_enable_factual.store(true);
        }
    }
    if (std::getenv("DKV_DENSE_CMP")) { run_dense_attn_cmp(); return 0; }
    if (std::getenv("DKV_RECON_CMP")) { run_recon_cmp(); return 0; }
    if (std::getenv("DKV_ATTN_CMP")) { run_attn_cmp(); return 0; }
    if (argc < 2) {
    ...
    if (std::getenv("DKV_VERBOSE") && std::string(std::getenv("DKV_VERBOSE")) == "1") {
        std::cerr << "[DKV Native] Initialized scheduler with backends: GPU=" 
    ...
    // Load from env vars
    if (const char* env = std::getenv("DKV_SRL_K_SEM")) srl_k_semantic = std::stoi(env);
    if (const char* env = std::getenv("DKV_SRL_K_LEX")) srl_k_lexical = std::stoi(env);
    if (const char* env = std::getenv("DKV_SRL_K_GRAPH")) srl_k_graph = std::stoi(env);
    if (const char* env = std::getenv("DKV_SRL_K_RECENCY")) srl_k_recency = std::stoi(env);
    if (const char* env = std::getenv("DKV_SRL_K_KEEP")) srl_k_keep = std::stoi(env);
    ...
    if (!env_tokens) {
        env_tokens = std::getenv("DKV_MAX_CTX_TK");
    }
    if (env_tokens) {
        int max_tokens = std::stoi(env_tokens);
        n_slots = (max_tokens + micro_block_size - 1) / micro_block_size;
        std::cerr << "[DKV Native] Overriding context limit from max_ctx_tk: " 
                  << max_tokens << " tokens (" << n_slots << " slots)" << std::endl;
        overridden = true;
    } else if (const char* env_slots = std::getenv("DKV_MAX_CONTEXT_SLOTS")) {
        n_slots = std::stoi(env_slots);
        std::cerr << "[DKV Native] Overriding context limit from DKV_MAX_CONTEXT_SLOTS: " 
                  << n_slots << " slots" << std::endl;
        overridden = true;
    }

    if (!overridden) {
        if (const char* env_preset = std::getenv("DKV_PRESET")) {
            std::string preset(env_preset);
            if (preset == "low") {
                n_slots = 4096 / micro_block_size; // 4096 tokens
                std::cerr << "[DKV Native] Preset 'low' detected: capping context size to 4096 tokens (" << n_slots << " slots)" << std::endl;
            } else if (preset == "mid") {
                n_slots = 8192 / micro_block_size; // 8192 tokens
                std::cerr << "[DKV Native] Preset 'mid' detected: capping context size to 8192 tokens (" << n_slots << " slots)" << std::endl;
            } else if (preset == "high") {
                n_slots = 16384 / micro_block_size; // 16384 tokens
                std::cerr << "[DKV Native] Preset 'high' detected: capping context size to 16384 tokens (" << n_slots << " slots)" << std::endl;
            } else {
                std::cerr << "[DKV Native] Using default model context length: " 
    ...
    int rank = 16;
    if (const char* env_rank = std::getenv("DKV_RANK")) {
    ...
    setenv("DKV_MAX_ACTIVE_DENSE_TOKENS", default_maxd.c_str(), 0);
    setenv("DKV_KV_QUANT", default_quant.c_str(), 0);

    ggml_type kv_quant_type = GGML_TYPE_Q8_0;
    if (const char* env_quant = std::getenv("DKV_KV_QUANT")) {
    ...
    if (const char* env_budget = std::getenv("DKV_GPU_BUDGET_GB")) {
    ...
    // (compress everything older than the recency window, like MLX). DKV_RECENCY_WINDOW
    // overrides the dense window for experiments / parity.
    int cfg_recency_window = 512;
    int cfg_short_context = 0;
    if (const char* e = std::getenv("DKV_RECENCY_WINDOW")) { try { cfg_recency_window = std::stoi(e); } catch (...) {} }
    if (const char* e = std::getenv("DKV_SHORT_CONTEXT")) { try { cfg_short_context = std::stoi(e); } catch (...) {} }
    dkv::KVRuntimeManager runtime_manager(rank, micro_block_size, gpu_budget_bytes,
    ...
    // explicit DKV_ENGAGE_THRESHOLD=8192 was unaffected, since the env-var
    ...
    if (const char* e = std::getenv("DKV_ENGAGE_THRESHOLD")) {
        try { dense_cap_engage = std::max(dense_cap_engage, std::stoi(e)); } catch (...) {}
    }
    int dense_cap_gen = 2048;     // DKV_MAX_TOKENS default
    if (const char* e = std::getenv("DKV_MAX_TOKENS")) {
        try { dense_cap_gen = std::max(1, std::stoi(e)); } catch (...) {}
    }
    int DENSE_WINDOW_CAP = dense_cap_engage + dense_cap_gen + 512;  // ~4608 default

    std::vector<dkv::AlignedFloatVector> active_k_dense(n_layers);
    std::vector<dkv::AlignedFloatVector> active_k_dense_rotated(n_layers);
    std::vector<dkv::AlignedFloatVector> active_v_dense(n_layers);
    dkv::AlignedInt32Vector active_positions_dense;
    ...
    if (const char* env_seed = std::getenv("DKV_SEED")) {
    ...
    if (const char* env_temp = std::getenv("DKV_TEMPERATURE")) {
        temperature = std::stof(env_temp);
    }
    if (const char* env_topp = std::getenv("DKV_TOP_P")) {
        top_p = std::stof(env_topp);
    }
    if (const char* env_rep = std::getenv("DKV_REPETITION_PENALTY")) {
    ...
    // ("7741-7741-"). DKV_REP_PENALTY_PROTECT_NUMERIC=0 restores.
    ...
    static const bool rep_protect_numeric = []() {
        const char* e = std::getenv("DKV_REP_PENALTY_PROTECT_NUMERIC");
    ...
    // with a request's own GPU dispatch. DKV_ENGAGE_THRESHOLD still overrides
    // (read once here too — it doesn't change between turns of the same process).
    int cached_engage_threshold = 0;  // 0 = auto
    if (const char* env_et = std::getenv("DKV_ENGAGE_THRESHOLD")) {
    ...
    if (cached_engage_threshold <= 0) {
        ...
        const char* env_frac = std::getenv("DKV_KV_MEM_FRAC");
        ...
        // Default engage point ~8k (matching MLX's DKV_COMPRESSED_MIN_CTX=8192
        ...
        // whenever DKV_ENGAGE_THRESHOLD is unset — i.e. every auto-resolved
        ...
        std::cerr << "[DKV] auto engage_threshold=" << cached_engage_threshold
    ...
    while (true) {
        ...
        if (std::getenv("DKV_DBG_DUMP_PROMPT")) {
        ...
        //
        // WHY: dkv_native has no dense KV cache (unlike ACTIVE_RUNTIME's PyTorch model
        ...
        if (do_full_reset || is_warmup_run) {
            ...
            srl_state.chunk_graph = dkv::ChunkGraph();
            srl_state.semantic_index = dkv::SemanticIndex();
        ...
        if (prompt.rfind("<|im_start|", 0) == 0) {
            ...
            chat_prompt = prompt;
            if (std::getenv("DKV_VERBOSE") && std::string(std::getenv("DKV_VERBOSE")) == "1") {
                std::cerr << "[DKV Native] Using pre-formatted chat prompt (" << prompt.size() << " bytes)" << std::endl;
            }
        } else {
            chat_prompt =
        ...
        if (!interactive) {
            std::cerr << "[DKV Native] Prompt: \"" << prompt << "\"" << std::endl;
        }
        ...
        if (true) {
            std::cerr << "[DKV Native] Prompt tokens (" << prompt_tokens.size() << "): ";
        ...
        // length-based adaptation (mlx_dkv_wrapper.py:1030, hf_dkv_wrapper.py:378). The
        ...
        if (const char* env_mt = std::getenv("DKV_MAX_TOKENS")) {
        ...
        // Compute raw slots needed for just the prompt (no headroom yet), then expand.
        {
            ...
            if (prompt_slots_needed > n_slots) {
                ...
                if (new_n_slots > n_slots) {
                    std::cerr << "[DKV Native] Auto-expanding pool: " << n_slots
        ...
        if (L > true_capacity) {
            ...
            std::cerr << "[DKV Native] Warning: Prompt (" << L << " tokens) exceeds model max capacity ("
        ...
        // in GPU memory. ggml_dkv_attn (sparse path) enables 1M+ contexts by
        ...
        if (const char* env_maxd = std::getenv("DKV_MAX_ACTIVE_DENSE_TOKENS")) {
        ...
        const char* env_maxd_val = std::getenv("DKV_MAX_ACTIVE_DENSE_TOKENS");
        ...
        if (cached_len > 0) {
            ...
            if (mismatch_idx >= cached_len && cached_len < L) {
                ...
                if (std::getenv("DKV_VERBOSE") && std::string(std::getenv("DKV_VERBOSE")) == "1") {
                    std::cerr << "[DKV Native] Prefix mismatch. Matching prefix length: " << mismatch_idx 
        ...
        if (!cache_valid) {
            if (std::getenv("DKV_VERBOSE") && std::string(std::getenv("DKV_VERBOSE")) == "1") {
                std::cerr << "[DKV Native] Warning: cached_len mismatch or invalid (" << cached_len 
        ...
        // ── 1. PREFILL PHASE ──
        if (!interactive) {
            std::cerr << "[DKV Native] Running Prefill phase in chunks..." << std::endl;
        }
        ...
        if (const char* env_pcs = std::getenv("DKV_PREFILL_CHUNK_SIZE")) {
            try { chunk_size = std::stoi(env_pcs); } catch (...) {}
        }

        // ── DKV_SPARSE_PREFILL knobs (hoisted above the host-mirror allocation:
        // both the LEGO ring geometry AND the mirror sizes below derive from them). ──
        bool sp_enabled = true;
        if (const char* e = std::getenv("DKV_SPARSE_PREFILL")) { try { sp_enabled = (std::stoi(e) != 0); } catch (...) {} }
        int sp_window = 1024, sp_sink_blocks = 1, sp_min = 2048;
        if (const char* e = std::getenv("DKV_SPARSE_PREFILL_WINDOW")) { try { sp_window = std::stoi(e); } catch (...) {} }
        if (const char* e = std::getenv("DKV_SPARSE_PREFILL_SINK_BLOCKS")) { try { sp_sink_blocks = std::stoi(e); } catch (...) {} }
        if (const char* e = std::getenv("DKV_SPARSE_PREFILL_MIN")) { try { sp_min = std::stoi(e); } catch (...) {} }
        const int sp_sink_end = sp_sink_blocks * micro_block_size;
        int sp_kmin = 8; float sp_frac = 0.05f; int sp_max_occ = 8;
        if (const char* e = std::getenv("DKV_SPARSE_PREFILL_KMIN")) { try { sp_kmin = std::stoi(e); } catch (...) {} }
        if (const char* e = std::getenv("DKV_SPARSE_PREFILL_FRAC")) { try { sp_frac = std::stof(e); } catch (...) {} }
        if (const char* e = std::getenv("DKV_SPARSE_PREFILL_MAX_OCC")) { try { sp_max_occ = std::stoi(e); } catch (...) {} }

        // ── DKV_LEGO_PREFILL (port of the MLX lego streaming prefill; see
        ...
        // prompt (cached_len == 0). DKV_LEGO_PREFILL=0 (default) = legacy.
        bool lego_on = false;
        if (const char* e = std::getenv("DKV_LEGO_PREFILL")) {
            try { lego_on = (std::stoi(e) != 0); } catch (...) {}
        }
        lego_on = lego_on && sp_enabled && decode_use_sparse && cached_len == 0;
        // ── DKV_LEGO_FAR: far-field mode under lego (all measured 2026-07-12,
        ...
        //   Requires uniform residual sets (DKV_RESIDUAL_UNIFORM=1 is set
        ...
        if (const char* e = std::getenv("DKV_LEGO_FAR")) {
            std::string m(e);
            if (m == "off") lego_far_mode = 0;
            else if (m == "recon") lego_far_mode = 2;
            else lego_far_mode = 1;
        }
        const int lego_mr_studs = std::min(dkv::NativeBlockPool::MAX_RESIDUAL, micro_block_size);
        ...
        if (lego_on) {
            ...
            if (lego_buf_rows + lego_far_rows >= L) {
                ...
                if (lego_far_mode == 1) {
                    ...
                    // DKV_LEGO_FAR note). Set BEFORE any of this prompt's blocks
                    // are submitted for compression; the compressor is idle here.
                    setenv("DKV_RESIDUAL_UNIFORM", "1", 1);
                }
                std::cerr << "[DKV] LEGO_PREFILL on: base_end=" << lego_base_end
        ...
        // equivalent in float16 (mlx_dkv_wrapper.py:345 "float16 explicitly to halve RAM").
        ...
        // DKV_SPARSE_PREFILL=0. Only engages above sp_min tokens, no-op below.
        ...
        if (sp_enabled) {
            ...
            std::cerr << "[DKV] SPARSE_PREFILL on: sink_end=" << sp_sink_end
        ...
        while (pos_start < L) {
            ...
            if (use_sp) {
                ...
                // (see the DKV_LEGO_FAR note at the lego config).
            ...
            if (lego_on && lego_n_far > 0) {
                ...
                for (int l = 0; l < n_layers; ++l) {
                    ...
                    dkv::NativeBlockPool* lg_pool = lg_engines[l].get();
                    for (int fi = 0; fi < (int)lego_far_blocks.size(); ++fi) {
                        ...
                        if (!emitted) {
                            ...
                            std::cerr << "[DKV] LEGO FATAL: far block " << bi << " layer " << l
        ...
        // ── DKV_DBG_EXPORT_CHECK: verify the raw-K export (k_activations, feeds the
        ...
        if (std::getenv("DKV_DBG_EXPORT_CHECK") && !is_warmup_run && persistent_k_cache[0] && !lego_on) {
            const int half_dim = head_dim / 2;
        ...
        // ── DKV_DBG_RECON_POS=<global token pos>: pool-reconstruction fidelity probe ──
        ...
        if (const char* env_rp = (lego_on ? nullptr : std::getenv("DKV_DBG_RECON_POS"))) {
        ...
        if (const char* env_force = std::getenv("DKV_FORCE_FIRST")) {
            first_decode_token = std::stoi(env_force);
            std::cerr << "[DKV DIAG] Forcing first token to: " << first_decode_token << " (\"" << model.token_to_piece(first_decode_token) << "\")\n";
        }
        {
            const auto& helper_ids = dkv::get_helper_token_ids_cpp(model);
            std::cerr << "[DKV DIAG] Total helper IDs in C++: " << helper_ids.size() << "\n";
        }
        ...
        dkv::SessionSRLState  srl_state_pending;
        ...
        std::thread srl_build_thread([&, _mbs, _desc_dim, _head_dim, _kv_heads, _L, relational_token_ids]() {
            ...
            if (n_comp > 0) {
                ...
                bool disable_factual = !g_dkv_enable_factual.load();
                if (lego_on && !disable_factual) {
                    ...
                    std::cerr << "[DKV] Factual store forced OFF under DKV_LEGO_PREFILL "
                                 "(reads ring-evicted raw activations).\n";
                    disable_factual = true;
                }
                try {
                  if (disable_factual) {
                    std::cerr << "[DKV] Factual store off (MLX turn-1 parity; DKV_ENABLE_FACTUAL=1 to build).\n";
                  } else {
                    ...
                    std::cerr << "[DKV] Factual store built (pre-decode): "
                              << srl_state_pending.factual_store.entries.size() << " entries.\n";
                  }
                } catch (const std::exception& fe) {
                    std::cerr << "[DKV] factual_store.build() in srl_build_thread failed: " << fe.what() << "\n";
                }

                std::cerr << "[DKV] SRL index ready: " << n_comp << " blocks." << std::endl;
            }
        ...
        // §3.3 fix: default raised from 2048 → 4096 to match ACTIVE_RUNTIME dkv_attention.py:75.
        // Comment from Python: "Default raised from 2048 → 4096 based on MPS benchmarks:
        //   ≤4K: DKV bypasses to pure dense. Dense handles these contexts fine without memory pressure.
        //   4K+: DKV engages. Decode is faster and VRAM is dramatically lower."
        ...
        if (decode_use_sparse) {
            ...
            if (n_comp_blocks == 0) {
                ...
                if (lego_on) {
                    ...
                    std::cerr << "[DKV] LEGO FATAL: sparse→dense fallback requested but the "
                                 "host mirrors are ring-sized (DKV_LEGO_PREFILL). Aborting "
                                 "this generation.\n";
                    if (interactive) {
                        std::cout << "__RESPONSE__" << std::endl;
                ...
                if (L >= engage_threshold) {
                    std::cerr << "[DKV] sparse→dense fallback (0 compressed blocks): backfilling "
        ...
        bool high_quality_routing = false;   // default: fast bounded-K pruning
        {
            ...
            if (n_comp_blocks > 0) {
                ...
                //
                // Toggle: DKV_HIGH_QUALITY_ROUTING = 1 → HQ (attend-all + graph)
                //                                        0 / unset → fast pruning (DEFAULT)
                //                                        auto → per-query heuristic (old behaviour:
                //                                               HQ for diffuse, fast for narrow)
                // DKV_MLX_PARITY stays as the low-level attend-all override for isolated
                ...
                if (const char* e = std::getenv("DKV_HIGH_QUALITY_ROUTING")) {
                    if (std::strcmp(e, "auto") == 0) {
                        high_quality_routing = !dkv_detect_narrow_query(prompt);
                    } else {
                ...
                if (const char* env_mp = std::getenv("DKV_MLX_PARITY")) {
                ...
                if (mlx_parity) {
                    ...
                    std::cerr << "[DKV] MLX parity active: srl_k_keep raised from " << srl_k_keep
                    ...
                    if (adaptive_k > srl_k_keep) {
                        std::cerr << "[DKV] §3.1 adaptive-k: srl_k_keep raised from " << srl_k_keep
                        ...
                        if (srl_k_semantic < sem_floor2) {
                            srl_k_semantic = sem_floor2;
                            std::cerr << "[DKV] §3.1 adaptive-k: srl_k_semantic raised to " << srl_k_semantic << "\n";
                        }
        ...
        if (n_slots <= 32) {
            ...
            if (target_k > srl_k_keep) {
                std::cerr << "[DKV] Short context dense fallback (n_slots <= 32): srl_k_keep raised from " << srl_k_keep
                          << " → " << target_k << "\n";
                srl_k_keep = target_k;
                int sem_floor2 = srl_k_keep * 3;
                if (srl_k_semantic < sem_floor2) {
        ...
        }
        {
            ...
            if (n_comp_blocks_for_dense <= 36 && n_comp_blocks_for_dense > 0) {
                ...
                target_k = std::min(target_k, 256);
                if (target_k > srl_k_keep) {
                    std::cerr << "[DKV] Short context dense fallback (n_comp_blocks <= 36): srl_k_keep raised from " << srl_k_keep
                              << " → " << target_k << "\n";
                    srl_k_keep = target_k;
                    int sem_floor2 = srl_k_keep * 3;
                    if (srl_k_semantic < sem_floor2) {
        ...
        if (std::getenv("DKV_VERBOSE") && std::string(std::getenv("DKV_VERBOSE")) == "1") {
            std::cerr << "[DKV Native] Building fresh decode graph..." << std::flush;
        }
        ...
        // per-token reconstruct-and-rotate path as the default. DKV_MPS_APPROXIMATE_ATTN
        // overrides in either direction.
        bool approx = (dkv::pool_rot_mode() == dkv::POOL_ROT_ABS);
        if (const char* env_approx = std::getenv("DKV_MPS_APPROXIMATE_ATTN")) {
            approx = (std::strcmp(env_approx, "1") == 0 || std::strcmp(env_approx, "true") == 0 || std::strcmp(env_approx, "yes") == 0 || std::strcmp(env_approx, "on") == 0);
        }

        std::vector<dkv::CustomAttnUserData> userdata(n_layers);
        for (int l = 0; l < n_layers; ++l) {
        ...
        // ── DKV_DECODE_CACHE: persistent F16 buffers for the materialized sparse cache.
        ...
        // DKV_CPU_EXACT_ATTN=1 forces the CPU callback path (execute_cpu_attention,
        ...
        static const bool cpu_exact_attn_env = (std::getenv("DKV_CPU_EXACT_ATTN") != nullptr);
        ...
        // subgraph MATH is exact under no-reuse via DKV_SELFTEST). Direct backend compute.
        bool factual_empty = true;
        if (!userdata.empty() && userdata[0].srl_state) {
            factual_empty = static_cast<dkv::SessionSRLState*>(userdata[0].srl_state)->factual_store.entries.empty();
        }
        ...
        // max_generate: default 2048, overridable via DKV_MAX_TOKENS env var or request
        max_generate = 2048;
        if (is_warmup_run) {
            max_generate = 2;
        } else if (const char* env_mt = std::getenv("DKV_MAX_TOKENS")) {
            max_generate = std::max(1, std::stoi(env_mt));
        }
        ...
        //
        // Env override: DKV_RETRIEVAL_INTERVAL=N (set to 1 to route every token)
        int retrieval_interval = 32;  // raised from 8 (matches Python cadence)
        if (const char* env_ri = std::getenv("DKV_RETRIEVAL_INTERVAL")) {
        ...
        // ── DKV_DECODE_CACHE persistent host state ──
        ...
        if (decode_use_sparse) {
            for (int l = 0; l < n_layers; ++l) {
                    ...
                    if (block->state == BlockState::DenseResident || block->state == BlockState::Compressing) {
                        ...
                        if (curr_token_idx + 1 > cap_tokens) {
                            ...
                            if (!warned_anchor && l == 0) {
                                warned_anchor = true;
                                std::cerr << "[DKV] WARNING: dense-window scan hit capacity ("
                        ...
                        if (!block->active_k.empty()) {
                            ...
                            if (active_len <= 0 || active_len > micro_block_size ||
                                ...
                                if (!warned_bad && l == 0) {
                                    warned_bad = true;
                                    std::cerr << "[DKV] WARNING: dense block with bad active_k len="
                ...
                if (l == 0 && std::getenv("DKV_VERBOSE")) {
                    std::cerr << "[DKV] dense-window scan: layer0 dense_blocks=" << dense_blocks_seen
        ...
        if (std::getenv("DKV_VERBOSE")) {
            ...
            std::cerr << "[DKV] dense-window positions: start=" << dense_start_positions[0]
        ...
        if (!decode_use_sparse) {
            ...
            // MLX (mlx_dkv_wrapper.py) keeps the most-recent (recency_window + block_size)
        ...
        const char* env_td = std::getenv("DKV_TIME_DECODE");
        bool time_decode = (env_td && std::string(env_td) == "1");

        // Bug 🅓 fix: n-gram loop detection state (mirrors mlx_dkv_wrapper.py:1204-1238)
        ...
        // DIAGNOSTIC (DKV_SCAN_POOL): after compression, scan every compressed slot for
        ...
        if (decode_use_sparse) {
            ...
            std::cerr << "[DKV] Prefill activations memory reclaimed early.\n";
        }
        ...
        for (int step = 0; step < max_generate; ++step) {
            ...
            if (!srl_swapped && srl_build_done.load(std::memory_order_acquire)) {
                ...
                srl_state = std::move(srl_state_pending);
                srl_swapped = true;
                std::cerr << "[DKV] SRL routing active from step " << step << std::endl;
                ...
                std::cerr << "[DKV] Prefill activations memory reclaimed early." << std::endl;
            }

            if (active_slot >= n_slots) {
                std::cerr << "\n[DKV Native] Warning: Context slot capacity reached during decode. Stopping generation." << std::endl;
                break;
            }
            ...
            if (std::getenv("DKV_DBG_POS")) { static int o=0; if(o++<3)
            ...
            if (do_retrieval) {
                ...
                if (step == 0 && !is_warmup_run && std::getenv("DKV_VERBOSE") && std::string(std::getenv("DKV_VERBOSE")) == "1") {
                    // Diagnostic: print retrieval results before first generated token
                    std::cerr << "\n[DKV DIAG] Decode step 0 retrieval:\n";
                ...
                if (std::getenv("DKV_ROUTING_VERBOSE")) {
            ...
            } else if (std::getenv("DKV_ROUTING_VERBOSE")) {
            ...
            if (decode_use_sparse) {
                ...
                for (auto sid : filtered_candidates) {
                    if (sid >= 0 && sid < n_pool_slots &&
                        state_table.get(sid) == dkv::BlockState::CompressedResident) {
                        fallback_sid = sid;
                        break;
                    }
                }
            ...
            static const bool dbg_candidates = (std::getenv("DKV_DBG_CANDIDATES") != nullptr);
            ...
            if (std::getenv("DKV_DBG_WINDOW") && step < 4 && !is_warmup_run) {
            ...
            if (native_attn_on && decode_use_sparse) {
                ...
                if (full_upload_needed || !persistent_buffers_initialized) {
                    if (std::getenv("DKV_VERBOSE") && std::string(std::getenv("DKV_VERBOSE")) == "1") {
                        std::cerr << "[DKV PERF] Initializing persistent GPU buffers (one-time upload: "
                                  << (n_layers * native_maxd * F_test * 2 * sizeof(float)) / (1024*1024) 
                                  << " MB)" << std::endl;
                    }
                    
                    if (std::getenv("DKV_DBG_SEL") && step == 0) {
                    ...
                    if (std::getenv("DKV_VERBOSE") && std::string(std::getenv("DKV_VERBOSE")) == "1") {
                        std::cerr << "[DKV PERF] Persistent buffers initialized. Subsequent tokens will upload only ~"
            ...
            // Fix: patch t_dense in op_params for all DKV_ATTN nodes each step.
            ...
            if (native_attn_on && decode_use_sparse) {
                ...
                for (int ni = 0; ni < N_tdfix; ++ni) {
                    ...
                    if (nd->op == GGML_OP_DKV_ATTN) {
                        struct ggml_dkv_attn_params dp;
            ...
            // ── DKV_DECODE_CACHE: fill the materialized sparse cache before the flash graph ──
            ...
            if (decode_cache_on && decode_use_sparse && cache_mask) {
                ...
                // many std::threads can oversubscribe. DKV_MAT_THREADS tunes it (default = full).
                static const int mat_threads = (std::getenv("DKV_MAT_THREADS") ? atoi(std::getenv("DKV_MAT_THREADS")) : nthreads);

                static double t_remat_ms = 0, t_win_ms = 0; static int t_fill_n = 0;
                const bool t_fill = (std::getenv("DKV_DBG_FILL_TIME") != nullptr);
                ...
                // DKV_DECODE_CACHE_ALWAYS_REMAT=1 restores the old unconditional periodic remat.
                static const bool force_periodic_remat =
                    (std::getenv("DKV_DECODE_CACHE_ALWAYS_REMAT") != nullptr);
                ...
                // DKV_DBG_CACHE_WINONLY / _ROUTEDONLY isolate the two halves for debugging.
                static const bool winonly    = (std::getenv("DKV_DBG_CACHE_WINONLY") != nullptr);
                static const bool routedonly = (std::getenv("DKV_DBG_CACHE_ROUTEDONLY") != nullptr);
                ...
                if (std::getenv("DKV_DBG_CACHE") && step < 3 && !is_warmup_run) {
            ...
            if (native_attn_on && std::getenv("DKV_DBG_NAN")) {
                int count = 0;
                for (int i = 0; i < decode_graph->n_nodes; ++i) {
                    struct ggml_tensor * node = decode_graph->nodes[i];
                    if (node->op == GGML_OP_DKV_ATTN) {
            ...
            if (std::getenv("DKV_DBG_GRAPH")) { static int o=0; if(o++<10)
                std::cerr << "[DBG_CBCOUNT] step=" << o << " sparse=" << decode_use_sparse
                          << " cb=" << dkv::g_dkv_cb_invocations.load()
                          << " cpu_attn=" << dkv::g_cpu_attn_count.load()
                          << " metal_attn=" << dkv::g_metal_attn_count.load() << "\n"; }
            ...
            // DEFINITIVE native-vs-CPU sparse check (run with DKV_DBG_DENSEOFF=1 so native attn_out
            ...
            if (std::getenv("DKV_DBG_CMP") && step < 12 && g_dbg_attn0 && g_dbg_qrope && g_dbg_sel0) {
                const auto & cfg = model.get_config();
                const char* clp = std::getenv("DKV_DBG_CMP_LAYER"); int cmpL = clp ? atoi(clp) : 0;
                ...
                dkv::execute_cpu_attention(qh.data(), sl.data(), outS.data(), lseS.data(), kv_engines[cmpL].get(),
                ...
                // (it's appended to active_k_dense after compute, for the NEXT step). DKV_DBG_CMP_CUR
                // re-adds it to A/B the effect.
                bool add_cur = std::getenv("DKV_DBG_CMP_CUR") != nullptr;
                ...
                dkv::cpu_dense_attention(qh.data(), dk.data(), dv.data(), dp.data(), Tc, nq, nkv, D, scale, true, freq, 0, outD.data(), lseD.data());
            ...
            // DKV_BLOCK_CMP: per-layer comparison table vs CPU reference
            if (std::getenv("DKV_BLOCK_CMP")) {
                const auto & cfg = model.get_config();
                int nq=cfg.n_head, nkv=cfg.n_head_kv, D=cfg.n_embd/cfg.n_head;
                float freq=cfg.rope_freq_base;
                float scale=1.0f/std::sqrt((float)D);
                int F=nkv*D;
                const char* slimit = std::getenv("DKV_BLOCK_CMP_STEPS");
                ...
                if (step < max_steps) {
                    ...
                    for (int l = 0; l < (int)kv_engines.size() && l < BLOCK_CMP_MAX_LAYERS; ++l) {
                        ...
                        dkv::execute_cpu_attention(qh.data(), sl.data(), outS.data(), lseS.data(),
                        ...
                        dkv::cpu_dense_attention(qh.data(), dk.data(), dv.data(), dp.data(), Tc,
            ...
            if (std::getenv("DKV_DBG_SEL") && step == 0 && dbg_anc) {
            ...
            if (std::getenv("DKV_DBG_SEL") && step == 0 && decode_selected_slots) {
            ...
            // DKV_DBG_STEP_LOGITS: top-5 logits for the first few decode steps —
            ...
            if (std::getenv("DKV_DBG_STEP_LOGITS") && step < 4 && !is_warmup_run) {
            ...
            if (userdata[0].layer0_q_tensor) {
                ...
                static const bool dbg_q = (std::getenv("DKV_DBG_Q") != nullptr);
                if (dbg_q) {
                    double sum_sq = 0.0;
            ...
            static const bool no_dense_slide = (std::getenv("DKV_NO_DENSE_SLIDE") != nullptr);
            ...
            if (!read_gpu_argmax) {
                ...
                // license). Default OFF, gated by DKV_RC8_LICENSE, and now
                ...
                // workloads can A/B it: DKV_RC8_LICENSE=1.
                static const bool rc8_license_on = []() {
                    const char* e = std::getenv("DKV_RC8_LICENSE");
                ...
                if (rc8_license_on && current_entity != -1 &&
                    ...
                    dkv::compute_entity_token_license(
                ...
                // to match mlx_dkv_wrapper.py which has no such guard.
                // Bug 🅘 fix.
                if (last_token >= 0 && alnum_cache[last_token] && !srl_state.current_step_factual_sequences.empty()) {
                    const auto& helper_ids_trans = dkv::get_helper_token_ids_cpp(model);
            ...
            // (mlx_dkv_wrapper.py:4514-4542). The previous single metric
            ...
            if (!loop_detected && (int)generated_tokens.size() >= 30 && (int)generated_tokens.size() % 10 == 0) {
                ...
                if (exact_loop || ratio_loop) {
                    ...
                    std::cerr << "\n[DKV Native] WARNING: repetition loop detected at step "
            ...
            if (loop_detected && loop_detected_idx >= 0 && (step - loop_detected_idx) >= 40) {
                std::cerr << "\n[DKV Native] WARNING: repetition loop persisted 40 tokens after detection — forcing EOS.\n";
                break;
            }

            if (!read_gpu_argmax) {
                // §3.7 fix: Skip non-alphanumeric tokens in rep penalty to match HF reference.
                // HF: hf_dkv_wrapper.py:904-912 skips tokens with no alphanumeric characters
                ...
                for (int32_t tok : unique_penalized) {
                    if (tok >= 0 && tok < n_vocab) {
                        ...
                        // (hf_dkv_wrapper.py:951-961, `if not is_alnum: continue`),
                ...
                if (const char* env_vsl = std::getenv("DKV_DISABLE_VSL")) {
                ...
                if (sfa_active && !srl_state.current_step_factual_sequences.empty()) {
                    const auto& helper_ids = dkv::get_helper_token_ids_cpp(model);
                    const auto& structural_ids = dkv::get_structural_helper_token_ids_cpp(model);
                    auto allowed = dkv::get_allowed_tokens_vsl_cpp(
            ...
            {
                const auto& helper_ids = dkv::get_helper_token_ids_cpp(model);
                dkv::update_vsl_state_cpp(next_token, srl_state, helper_ids, model);
                ...
                // DKV_DBG_RECON_ERR (ruled out — reconstruction error was flat
                // and normal) + DKV_VERBOSE: factual_store.query() returned
            ...
            // §3.6 fix: gate raised 0.4→0.5 to match MLX reference (mlx_dkv_wrapper.py).
            ...
            if (!srl_state.factual_store.entries.empty()) {
                ...
                // HF reference: dkv_attention.py:771-777 uses threshold=0.50 and
                ...
                for (const auto& hit : fact_hits) {
                    ...
                    // §3.4 (§5) fix: thresholds aligned with MLX reference (mlx_dkv_wrapper.py):
                ...
                dkv::process_and_tag_vsl_step(srl_state);
            ...
            if (time_decode) {
                ...
                double metal_wait_ms = dkv::get_and_reset_accumulated_wait_ms();
            ...
            if (active_block_tokens == 0 && active_slot > old_active_slot) {
                ...
                if (prev_slot >= 0 && prev_slot < (int)blocks.size()) {
                    ...
                    // dkv_native was doing this every 16 tokens → O(N²) CBLAS sgemm spike
            ...
            // bounded by the MLX-parity slide. Restore with DKV_LEGACY_WINDOW_REBUILD=1.
            static const bool legacy_window_rebuild = (std::getenv("DKV_LEGACY_WINDOW_REBUILD") != nullptr);
        ...
        if (profile_steps > 0 && std::getenv("DKV_PROFILE") && std::string(std::getenv("DKV_PROFILE")) == "1") {
            double total_profile_ms = sum_sync_ms + sum_recon_ms + sum_attn_ms + sum_graph_other_ms + sum_sampling_ms + sum_other_ms;
            std::cerr << "\n==================================================\n";
            std::cerr << "       DKV NATIVE PROFILE BREAKDOWN\n";
            ...
            std::cerr << "==================================================\n\n";
            dkv::print_rank_energy_histogram();
        }
        if (!is_warmup_run && std::getenv("DKV_DBG_GRAPH")) {
            std::cerr << "[DBG_CBTOTAL] custom_attention_op_callback total invocations this response = "
                      << dkv::g_dkv_cb_invocations.load() << "\n";
        }
        if (!is_warmup_run) {
            std::cout << std::endl;
            if (interactive) {
        ...
        try {
            ...
            std::cerr << "[DKV Native] Error in commit_turn on turn boundary: " << e.what() << std::endl;
        }
        ...
        // matching ACTIVE_RUNTIME mlx_dkv_wrapper.py / kv_runtime_manager.py:955.
        ...
        for (int l = 0; l < n_layers; ++l) {
            dkv::AlignedFloatVector().swap(active_k_dense[l]);
            dkv::AlignedFloatVector().swap(active_k_dense_rotated[l]);
            dkv::AlignedFloatVector().swap(active_v_dense[l]);
        }
        ...
        if (std::getenv("DKV_VERBOSE") && std::string(std::getenv("DKV_VERBOSE")) == "1") {
            std::cerr << "[DKV Native] Session prefix updated: " << session_cached_len
    ...
    dkv::cleanup_metal_attention();

    std::cerr << "[DKV Native] Text generation completed successfully!" << std::endl;
    return 0;
}
