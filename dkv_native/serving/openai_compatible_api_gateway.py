...
app = FastAPI(title="DKV C++ Native API Server")
...
# Resolve paths relative to this file (…/dkv_native/serving/ → …/dkv_native/)
...
BINARY_PATH_DEFAULT = os.environ.get(
    "DKV_BINARY_PATH", os.path.join(_NATIVE_ROOT, "build", "dkv_native"))
MODEL_PATH_DEFAULT  = os.environ.get(
    "DKV_MODEL_PATH", _model_path("qwen2.5-0.5b-instruct.gguf"))
...
class SubprocessWrapper:
    ...
    def start(self):
        ...
        binary_path = os.getenv("DKV_BINARY_PATH", BINARY_PATH_DEFAULT)
        model_path  = os.getenv("DKV_MODEL_PATH",  MODEL_PATH_DEFAULT)

        print(f"[Server] Launching C++ subprocess: {binary_path} {model_path} -")
        
        self.verbose = os.environ.get("DKV_VERBOSE") == "1"
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description="DKV C++ Native API Server")
    ...
    if args.preset == 'low':
        os.environ["DKV_PREFILL_CHUNK_SIZE"] = "256"
    elif args.preset == 'high':
        os.environ["DKV_PREFILL_CHUNK_SIZE"] = "2048"
    else:
        os.environ["DKV_PREFILL_CHUNK_SIZE"] = "512"

    os.environ["DKV_MAX_TOKENS"] = str(args.max_tokens)

    if "DKV_USE_GPU" not in os.environ:
        os.environ["DKV_USE_GPU"] = "1"
    ...
    if "DKV_MODEL_PATH" not in os.environ:
        model_arg = args.model
        if model_arg.endswith(".gguf") and os.path.exists(model_arg):
            os.environ["DKV_MODEL_PATH"] = os.path.abspath(model_arg)
        elif "0.5b" in model_arg.lower():
            os.environ["DKV_MODEL_PATH"] = _model_path("qwen2.5-0.5b-instruct.gguf")
        elif "1.5b" in model_arg.lower():
            q4_path = _model_path("qwen2.5-1.5b-instruct-q4_k_m.gguf")
            if os.path.exists(q4_path):
                os.environ["DKV_MODEL_PATH"] = q4_path
            else:
                os.environ["DKV_MODEL_PATH"] = _model_path("qwen2.5-1.5b-instruct-q8_0.gguf")
        else:
            os.environ["DKV_MODEL_PATH"] = MODEL_PATH_DEFAULT

    print(f"[DKV Native Server] Starting:")
    print(f"  Model:      {os.environ.get('DKV_MODEL_PATH')}")
    print(f"  Chunk size: {os.environ.get('DKV_PREFILL_CHUNK_SIZE')}")
    print(f"  Max tokens: {os.environ.get('DKV_MAX_TOKENS')}")
