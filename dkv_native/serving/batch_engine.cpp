...
static int32_t sample_token(
    ...
    if (repetition_penalty != 1.0f && model != nullptr) {
...
void DKVBatchEngine::start() {
    if (running_.exchange(true)) return;
    worker_thread_ = std::thread(&DKVBatchEngine::run_loop, this);
}

void DKVBatchEngine::stop() {
...
std::shared_ptr<BatchRequest> DKVBatchEngine::submit(
...
void DKVBatchEngine::cancel(const std::string& session_id) {
    std::lock_guard<std::mutex> lock(queue_mutex_);
    ...
    while (running_) {
        ...
        if (req && !req->cancelled) {
            try {
                ...
                std::cerr << "[DKV BatchEngine] Exception during request execution: " << e.what() << std::endl;
                req->set_error(std::string("Internal batch engine exception: ") + e.what());
            } catch (...) {
                std::cerr << "[DKV BatchEngine] Unknown exception during request execution" << std::endl;
...
static bool dkv_detect_narrow_query(const std::string& prompt) {
...
void DKVBatchEngine::process_request(const std::shared_ptr<BatchRequest>& req) {
    if (!session_manager_) {
    ...
    // Configurable via DKV_MAX_DENSE_TOKENS env var.
    int max_dense_cap = engage_threshold;  // default = 2048
    if (const char* env_mdc = std::getenv("DKV_MAX_DENSE_TOKENS")) {
    ...
    if (!match || prefill_offset == 0) {
        ...
        session->srl_state.chunk_graph = dkv::ChunkGraph();
        session->srl_state.semantic_index = dkv::SemanticIndex();
    ...
    // SRL Routing Budget scaling + dense fallback matching main.cpp
    {
        ...
        if (n_comp_blocks > 0) {
            bool mlx_parity = !dkv_detect_narrow_query(req->prompt);
    ...
    session->srl_state.factual_store.clear();
    ...
    session->srl_state.factual_anchor_w = 0.80f;

    auto & blocks_layer0 = runtime_manager_->get_ingest_manager().get_blocks(0);
    std::vector<int> compressed_slots;
    ...
    for (int i = 0; i < (int)blocks_layer0.size(); ++i) {
        if (blocks_layer0[i]->pool_idx != -1 &&
            (blocks_layer0[i]->state == BlockState::CompressedResident ||
             ...
             blocks_layer0[i]->state == BlockState::DenseResident ||
             blocks_layer0[i]->state == BlockState::Compressing)) {
    ...
    if (completed_blocks > 0) {
        ...
        // Populate current_query_tokens (query tokens since cached_len)
        session->srl_state.current_query_tokens.clear();
        if (cached_len < L) {
            session->srl_state.current_query_tokens.assign(prompt_tokens.begin() + cached_len, prompt_tokens.end());
        }
        ...
        session->srl_state.factual_store.build(
            ...
            [&](int32_t tid) { return model_->token_to_piece(tid); },
    ...
    // Default = 32 steps (raised from 8). Override: DKV_RETRIEVAL_INTERVAL=N
    ...
    for (int step = 1; step < req->max_tokens; ++step) {
        ...
        if (do_retrieval) {
            ...
            // mirrors ACTIVE_RUNTIME dkv_attention.py:544.
        ...
        if (srl_state.current_step_max_similarity >= 0.40f &&
            ...
            const auto& helper_ids_penalty = dkv::get_helper_token_ids_cpp(*model_);
        ...
        if (last_token >= 0 && is_alnum_trans && !srl_state.current_step_factual_sequences.empty()) {
            const auto& helper_ids_trans = dkv::get_helper_token_ids_cpp(*model_);
        ...
        if (req->sfa_active) {
            const auto& helper_ids = dkv::get_helper_token_ids_cpp(*model_);
            const auto& structural_ids = dkv::get_structural_helper_token_ids_cpp(*model_);
        ...
        {
            const auto& helper_ids = dkv::get_helper_token_ids_cpp(*model_);
            dkv::update_vsl_state_cpp(next_token, session->srl_state, helper_ids, *model_);
        ...
        if (!session->srl_state.factual_store.entries.empty()) {
            ...
            if (!decode_q.empty()) {
                q_for_factual.resize(model_->get_config().n_embd);
                query_heads = model_->get_config().n_head;
                if (session->srl_state.factual_anchor_q.empty()) {
                    session->srl_state.factual_anchor_q = decode_q;
                    session->srl_state.factual_anchor_w = 0.80f;
                    ...
                    for (int qi = 0; qi < (int)model_->get_config().n_embd; ++qi) {
                ...
                if (session->srl_state.factual_anchor_q.empty()) {
                    ...
                    int anchor_size = (int)session->srl_state.factual_anchor_q.size();
                    q_for_factual.resize(anchor_size);
                    const auto& current_source = (anchor_size == (int)model_->get_config().n_embd) ? decode_q : decode_k[0];
                    query_heads = (anchor_size == (int)model_->get_config().n_embd) ? model_->get_config().n_head : kv_heads;
                    if (!current_source.empty()) {
                        ...
                        q_for_factual = session->srl_state.factual_anchor_q;
                    }
                }
            }
            ...
            dkv::process_and_tag_vsl_step(session->srl_state);
        }
                        ...
                        for (int i = 0; i < (int)all_blocks_r.size(); ++i) {
                            if (all_blocks_r[i]->pool_idx != -1 &&
                                (all_blocks_r[i]->state == BlockState::CompressedResident ||
                                 ...
                                 all_blocks_r[i]->state == BlockState::DenseResident ||
        ...
        if (!req->repetition_loop_detected && n_new >= 30 && n_new % 10 == 0) {
            ...
            if (window_size >= 6) {
                ...
                if (total_ngrams > 0 && (float)max_count / total_ngrams >= 0.35f) {
                    ...
                    std::cerr << "[DKV C++] WARNING: repetition loop detected for session "
        ...
        if (req->repetition_loop_detected && n_new - req->loop_detection_idx >= 40) {
            std::cerr << "[DKV C++] WARNING: repetition loop for session " << req->session_id
    ...
    try {
        ...
        std::cerr << "[DKV C++] Error in commit_turn on turn boundary: " << e.what() << std::endl;
    }
