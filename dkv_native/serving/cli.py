...
async def run_client_mode(args):
    ...
    print_system(f"Connecting to Native DKV Gateway at {COLOR_BOLD}{api_url}{COLOR_RESET}...")
...
async def run_direct_mode(args):
    ...
    # (DKV_COMPRESSOR_THREADS=N) — those threads are background-QoS and yield to the
    ...
    if "DKV_COMPRESSOR_THREADS" not in os.environ:
        os.environ["DKV_COMPRESSOR_THREADS"] = "4"
    ...
    if hasattr(args, 'context') and args.context is not None:
        os.environ["DKV_MAX_CTX_TK"] = str(args.context)
    ...
    os.environ["DKV_TEMPERATURE"] = str(args.temperature)

    os.environ["DKV_TOP_P"] = str(args.top_p)
    os.environ["DKV_REPETITION_PENALTY"] = str(args.repetition_penalty)

    # Start wrapper
    wrapper = SubprocessWrapper(os.environ["DKV_BINARY_PATH"], os.environ["DKV_MODEL_PATH"])
    try:
        wrapper.start()
    except Exception as e:
        print_error(f"Failed to start C++ process: {e}")
        print_error(f"Make sure you have built dkv_native and that the binary exists at {os.environ['DKV_BINARY_PATH']}")
        return
    ...
    while True:
        ...
        if user_prompt_stripped.startswith("/") and ("\n" not in user_prompt_stripped or _first_tok == "/file"):
            ...
            if cmd in ["/exit", "/quit"]:
                ...
                print(f"Binary Path:     {os.environ['DKV_BINARY_PATH']}")
                print(f"Model Path:      {os.environ['DKV_MODEL_PATH']}")
                print(f"GPU Active:      {os.environ['DKV_USE_GPU'] == '1'}")
                print(f"Chunk Size:      {os.environ['DKV_PREFILL_CHUNK_SIZE']}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="dkv_native CLI: Interactive terminal interface for native C++ DKV.")
    ...
    parser.add_argument('--binary-path', type=str,
                        default=os.path.abspath(os.path.join(os.path.dirname(__file__), '../build/dkv_native')),
