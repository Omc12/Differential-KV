...
#define GGML_MAX_SRC            24  // dkv_native: raised to 24 for GGML_OP_DKV_ATTN with residuals (22 inputs)
...
extern "C" {
    ...
    enum ggml_op {
        ...
        GGML_OP_DKV_ATTN,  // dkv_native: fused compressed-KV decode attention (Metal only)
    ...
    // dkv_native: fused compressed-KV decode attention (Metal-only custom op).
    // Replaces the ~88-op build_native_sparse_attn subgraph with one fused kernel so it runs
    // in the single decode command buffer. Math identical to ACTIVE_RUNTIME
    // compute_decode_attention_static / dkv_decode.metal (anchor + low-rank delta, online
    // softmax over sparse∪dense∪current). Scalars travel in op_params.
    struct ggml_dkv_attn_params {
        int32_t n_q_heads;
        int32_t n_kv_heads;
        int32_t rank;
        int32_t s_max;            // pool S_max (per-slot token capacity)
        int32_t head_dim;         // D
        int32_t n_blocks;         // K (number of selected slots)
        int32_t max_dense;        // MAXD (dense buffer capacity)
        int32_t t_dense;          // active dense-window length
        float   scale;            // 1/sqrt(D)
        int32_t has_rope;
        float   rope_freq_base;
        int32_t approximate_attn;
        int32_t n_slots;          // pool slot count (for dedup/range-skip of selected slots)
        int32_t max_residual;     // max residual count per block
        int32_t is_q8;
    };

    GGML_API struct ggml_tensor * ggml_dkv_attn(
            struct ggml_context * ctx,
            struct ggml_tensor  * q,                 // [n_embd] rotated query
            struct ggml_tensor  * slot_indices,      // [K] i32
            struct ggml_tensor  * u_pool,            // pool device tensors ↓
            struct ggml_tensor  * u_scale,
            struct ggml_tensor  * vk_pool,
            struct ggml_tensor  * vv_pool,
            struct ggml_tensor  * anchors_k,
            struct ggml_tensor  * anchors_v,
            struct ggml_tensor  * seq_lens,
            struct ggml_tensor  * scales,
            struct ggml_tensor  * anchor_positions,
            struct ggml_tensor  * dense_k,           // [F_kv, MAXD] rotated-in-kernel
            struct ggml_tensor  * dense_v,
            struct ggml_tensor  * dense_pos,         // [MAXD] i32
            struct ggml_tensor  * dense_mask,        // [MAXD] f32 additive (0 valid / -inf invalid)
            struct ggml_tensor  * cur_k,             // [F_kv] current-token raw K (rotated-in-kernel)
            struct ggml_tensor  * cur_v,             // [F_kv] current-token V
            struct ggml_tensor  * cur_pos,           // [1] i32 current position
            struct ggml_tensor  * res_k_pos,         // [MAX_RESIDUAL, n_slots] i32
            struct ggml_tensor  * res_v_pos,         // [MAX_RESIDUAL, n_slots] i32
            struct ggml_tensor  * res_k_val,         // [D, n_kv, MAX_RESIDUAL, n_slots] f16
            struct ggml_tensor  * res_v_val,         // [D, n_kv, MAX_RESIDUAL, n_slots] f16
            struct ggml_tensor  * token_positions,   // [S_max, n_slots] i32
            struct ggml_dkv_attn_params params);
