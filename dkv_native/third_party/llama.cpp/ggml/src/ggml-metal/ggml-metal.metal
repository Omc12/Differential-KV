...
kernel void kernel_get_rows_q(
    ...
    const int32_t r_in = ((const device int32_t *) ((const device char *) src1 + i12*args.nb12 + i11*args.nb11 + i10*args.nb10))[0];
    const int32_t r = (r_in < 0 || r_in >= args.ne01) ? 0 : r_in;
...
// ───────────────────────────────────────────────────────────────────────────────────────────
// dkv_native: FLASH-DECODING fused compressed-KV decode attention (GGML_OP_DKV_ATTN).
// Two kernels: _partial (head×split threadgroups → per-split partials) + _combine (merge).
// Math == ACTIVE_RUNTIME compute_decode_attention_static. Scratch[(head*NSPLIT+split)*(D+2)] =
// {m_local, d_local, O_local[D]}. occupancy = n_q_heads*NSPLIT threadgroups.
// ───────────────────────────────────────────────────────────────────────────────────────────
// DKV_NSPLIT is defined in ggml-metal-impl.h (single source of truth).

kernel void kernel_dkv_attn_partial(
        constant ggml_metal_kargs_dkv_attn & args [[buffer(0)]],
        device const float * q          [[buffer(1)]],
        device const int   * slot_idx   [[buffer(2)]],
        device const char  * U_pool     [[buffer(3)]],
        device const half  * U_scale    [[buffer(4)]],
        device const void * VK_pool    [[buffer(5)]],
        device const void * VV_pool    [[buffer(6)]],
        device const void * anchors_K  [[buffer(7)]],
        device const void * anchors_V  [[buffer(8)]],
        device const int   * seq_lens   [[buffer(9)]],
        device const half  * scales     [[buffer(10)]],
        device const int   * anc_pos    [[buffer(11)]],
        device const float * dense_K    [[buffer(12)]],
        device const float * dense_V    [[buffer(13)]],
        device const int   * dense_pos  [[buffer(14)]],
        device const float * dense_mask [[buffer(15)]],
        device const float * cur_k      [[buffer(16)]],
        device const float * cur_v      [[buffer(17)]],
        device const int   * cur_pos    [[buffer(18)]],
        device const int   * res_K_pos  [[buffer(19)]],
        device const int   * res_V_pos  [[buffer(20)]],
        device const void * res_K_val  [[buffer(21)]],
        device const void * res_V_val  [[buffer(22)]],
        device const int   * token_positions [[buffer(23)]],
        device       float * scratch    [[buffer(24)]],
        uint3   tgpig [[threadgroup_position_in_grid]],
        ushort3 tpitg [[thread_position_in_threadgroup]],
        ushort3 ntg3  [[threads_per_threadgroup]],
        uint3   tg_grid_size [[threadgroups_per_grid]]) {
#define READ_VAL(buf, idx) ((args.is_q8) ? ((float)((device const block_q8_0 *)(buf))[(idx) / 32].d * (float)((device const block_q8_0 *)(buf))[(idx) / 32].qs[(idx) % 32]) : (float)((device const half *)(buf))[(idx)])
    const uint  tid     = tpitg.x;
    const uint  ntg     = ntg3.x;
    const int   head    = (int)tgpig.x;
    const int   split   = (int)tgpig.y;
    const int   runtime_nsplit = (int)tg_grid_size.y;
    const int   D       = args.head_dim;
    const int   half_d  = D / 2;
    const int   rank    = args.rank;
    const int   K       = args.n_blocks;
    const int   S_max   = args.s_max;
    const int   n_kv    = args.n_kv_heads;
    const int   n_slots = args.n_slots;
    const int   group   = args.n_q_heads / max(n_kv, 1);
    const int   kv_head = head / max(group, 1);
    const float scale   = args.scale;
    const bool  hr      = args.has_rope != 0;
    const float freq    = args.rope_freq_base;
    threadgroup int shared_T_dense;
    if (tid == 0) {
        int td = 0;
        while (td < args.t_dense && dense_mask[td] > -1e29f) {
            td++;
        }
        shared_T_dense = td;
    }
    threadgroup_barrier(mem_flags::mem_threadgroup);
    const int T_dense = shared_T_dense;


    threadgroup float q_sh[128];
    threadgroup float akr[128];
    threadgroup float qproj[64];
    threadgroup float red[256];
    threadgroup float red2[256];
    threadgroup float wp[64];
    threadgroup float dw[2048];
    threadgroup uchar skip[256];   // precomputed slot skip mask (range + dedup), K ≤ 256
    threadgroup float cache_anc[32];      // PASS1 anchor scores per split-block (reused in PASS2)
    threadgroup float cache_qp[32*64];    // PASS1 q_proj per split-block [j*rank + r]
    threadgroup float w_sh[256];          // Softmax weights for residual value accumulation

    // Zero-init dw[] before PASS2 writes it. The write loop strides by (DKV_NSPLIT * ntg)
    // but the accumulation read loop strides by DKV_NSPLIT only, so slots not written by
    // this thread's split contain GPU scratch garbage at short contexts (4k/16k) where
    // T_dense < 2048 and many threads participate in zero iterations of the write loop.
    for (int t = (int)tid; t < 2048; t += (int)ntg) dw[t] = 0.0f;
    threadgroup_barrier(mem_flags::mem_threadgroup);

    for (int d = (int)tid; d < D; d += (int)ntg) q_sh[d] = q[head * D + d];
    // Precompute skip[k] ONCE (thread-split) instead of O(K^2) per-thread per-block-loop.
    for (int k = (int)tid; k < K; k += (int)ntg) {
        int slot = slot_idx[k];
        bool sk = (slot < 0 || slot >= n_slots);
        if (!sk) { for (int j=0;j<k;++j) if (slot_idx[j]==slot) { sk=true; break; } }
        skip[k] = sk ? 1 : 0;
    }
    threadgroup_barrier(mem_flags::mem_threadgroup);

    // ── PASS 1: local online softmax (m,d) over THIS split's blocks (+dense/current on split 0) ──
    float m_i = -1e30f, d_i = 0.0f;
    #define ONLINE(s) { float _s=(s); if(_s>m_i){ d_i*=exp(m_i-_s); m_i=_s; } d_i += exp(_s-m_i); }
    for (int k = split; k < K; k += runtime_nsplit) {
        int slot = slot_idx[k];
        if (skip[k]) continue;
        int slen = seq_lens[slot];
        
        // Compute anchor score (PASS1 only uses anchor projection)
        float ta = 0.0f;
        for (int d = (int)tid; d < D; d += (int)ntg) {
            float kr = READ_VAL(anchors_K, slot * n_kv * D + kv_head * D + d);
            ta += q_sh[d] * kr;
        }
        red[tid] = ta; threadgroup_barrier(mem_flags::mem_threadgroup);
        for(uint st=ntg/2;st>0;st/=2){ if(tid<st) red[tid]+=red[tid+st]; threadgroup_barrier(mem_flags::mem_threadgroup);}
        float score_anc = red[0] * scale;
        threadgroup_barrier(mem_flags::mem_threadgroup);
        int jc=(k-split)/runtime_nsplit;
        if(tid==0) cache_anc[jc]=score_anc;
        
        float bsc=(float)scales[slot];
        if(tid==0) ONLINE(score_anc);
        
        if (args.approximate_attn != 0) {
            if (tid < (uint)rank) {
                float proj_val = 0.0f;
                int base_vk = slot * rank * n_kv * D + (int)tid * n_kv * D + kv_head * D;
                for (int d = 0; d < D; ++d) {
                    proj_val += q_sh[d] * READ_VAL(VK_pool, base_vk + d);
                }
                cache_qp[jc * rank + (int)tid] = proj_val * scale;
            }
            threadgroup_barrier(mem_flags::mem_threadgroup);

            for (int t = (int)tid; t < slen; t += (int)ntg) {
                float delta = 0.0f;
                int uoff = slot * S_max * rank + t * rank;
                for (int r = 0; r < rank; ++r) {
                    delta += cache_qp[jc * rank + r] * (float)U_pool[uoff + r];
                }
                float scu = (float)U_scale[slot * S_max + t];
                float t_score = score_anc + delta * scu * bsc;

                int ri = -1;
                int max_residual = args.max_residual;
                for (int r = 0; r < max_residual; ++r) {
                    if (res_K_pos[slot * max_residual + r] == t) {
                        ri = r;
                        break;
                    }
                }
                if (ri != -1) {
                    ...
                    int base_res = slot * max_residual * n_kv * D + ri * n_kv * D + kv_head * D;
                    for (int d = 0; d < D; ++d) {
                        res_score += q_sh[d] * READ_VAL(res_K_val, base_res + d);
                    }
            ...
            for (int t = (int)tid; t < slen; t += (int)ntg) {
                ...
                for (int d = 0; d < half_d; ++d) {
                    float raw_k1 = READ_VAL(anchors_K, slot * n_kv * D + kv_head * D + d);
                    float delta_k1 = 0.0f;
                    int vk_base1 = slot * rank * n_kv * D + kv_head * D + d;
                    for (int r = 0; r < rank; ++r) {
                        delta_k1 += (float)U_pool[uoff + r] * READ_VAL(VK_pool, vk_base1 + r * n_kv * D);
                    }
                    float k_raw1 = raw_k1 + delta_k1 * scu * bsc;

                    int d2 = d + half_d;
                    float raw_k2 = READ_VAL(anchors_K, slot * n_kv * D + kv_head * D + d2);
                    float delta_k2 = 0.0f;
                    int vk_base2 = slot * rank * n_kv * D + kv_head * D + d2;
                    for (int r = 0; r < rank; ++r) {
                        delta_k2 += (float)U_pool[uoff + r] * READ_VAL(VK_pool, vk_base2 + r * n_kv * D);
                    }
                    float k_raw2 = raw_k2 + delta_k2 * scu * bsc;
                    ...
                    float k_rot1 = k_raw1;
                    float k_rot2 = k_raw2;
                    delta_score += q_sh[d] * k_rot1 + q_sh[d2] * k_rot2;
                }
    ...
    // dense distributed across splits (strided): split s owns tokens t ≡ s (mod nsplit)
    for (int t = split + runtime_nsplit*(int)tid; t < T_dense; t += runtime_nsplit*(int)ntg) {
        if (dense_mask[t] < -1e29f) continue;
        int pos=dense_pos[t]; int bk=t*n_kv*D+kv_head*D;
        device const float4 * dense_K4 = (device const float4 *)(dense_K + bk);
        threadgroup const float4 * q_sh4 = (threadgroup const float4 *)q_sh;
        float sc = 0.0f;
        for (int d4 = 0; d4 < D / 4; ++d4) {
            float4 kr4 = dense_K4[d4];
            float4 q4 = q_sh4[d4];
            sc += kr4.x * q4.x + kr4.y * q4.y + kr4.z * q4.z + kr4.w * q4.w;
        }
        ONLINE(sc*scale);
    }
    if (split==0 && tid==0) {
        ...
        float sc=0.0f;
        for (int d = 0; d < half_d; ++d) {
            float k_raw1 = (float)cur_k[bk + d];
            float k_raw2 = (float)cur_k[bk + d + half_d];
            float k_rot1 = k_raw1;
            float k_rot2 = k_raw2;
            if (hr) {
                float theta = 1.0f / pow(freq, (2.0f * d) / D);
                float angle = pos * theta;
                float c = cos(angle), s = sin(angle);
                k_rot1 = k_raw1 * c - k_raw2 * s;
                k_rot2 = k_raw2 * c + k_raw1 * s;
            }
            sc += q_sh[d] * k_rot1 + q_sh[d + half_d] * k_rot2;
        }
        ONLINE(sc*scale);
    }
    red[tid]=m_i; red2[tid]=d_i; threadgroup_barrier(mem_flags::mem_threadgroup);
    for(uint st=ntg/2;st>0;st/=2){ if(tid<st){ float m1=red[tid],m2=red[tid+st],d1=red2[tid],d2=red2[tid+st];
        float mm=max(m1,m2); red[tid]=mm; red2[tid]=d1*exp(m1-mm)+d2*exp(m2-mm);} threadgroup_barrier(mem_flags::mem_threadgroup);} 
    float ml=red[0], dl=red2[0];
    threadgroup_barrier(mem_flags::mem_threadgroup);

    // ── PASS 2: O_local = Σ exp(score - ml)·V over this split (UNnormalized) ──
    float val[2] = {0.0f};
    for (int k=split; k<K; k+=runtime_nsplit) {
        int slot=slot_idx[k];
        if (skip[k]) continue;
        ...
        float bsc=(float)scales[slot];
        int jc=(k-split)/runtime_nsplit;
        float score_anc=cache_anc[jc];
        float w_anc=exp(score_anc-ml);
        float lsw=0.0f; float lwp[32]={0.0f};
        if (args.approximate_attn != 0) {
            for (int t = (int)tid; t < slen; t += (int)ntg) {
                float delta = 0.0f;
                int uoff = slot * S_max * rank + t * rank;
                ...
                float w = exp(t_score - ml);
                lsw += w;
                w_sh[t] = w;
                for (int r = 0; r < rank; ++r) {
                    lwp[r] += w * (float)U_pool[uoff + r] * scu;
                }
            }
        } else {
            for (int t = (int)tid; t < slen; t += (int)ntg) {
        ...
        red[tid]=lsw; threadgroup_barrier(mem_flags::mem_threadgroup);
        for(uint st=ntg/2;st>0;st/=2){ if(tid<st) red[tid]+=red[tid+st]; threadgroup_barrier(mem_flags::mem_threadgroup);} 
        float sumw=red[0]; threadgroup_barrier(mem_flags::mem_threadgroup);
        for (int r = 0; r < rank; ++r) {
            dw[(int)tid * rank + r] = lwp[r];
        }
        threadgroup_barrier(mem_flags::mem_threadgroup);
        if (tid < (uint)rank) {
            float sum_r = 0.0f;
            for (int t = 0; t < (int)ntg; ++t) {
                sum_r += dw[t * rank + (int)tid];
            }
            wp[(int)tid] = sum_r;
        }
        threadgroup_barrier(mem_flags::mem_threadgroup);
        float wtot=w_anc+sumw;
        for(int d=(int)tid;d<D;d+=(int)ntg){
            int idx = (d - (int)tid) / (int)ntg;
            val[idx]+=wtot * READ_VAL(anchors_V, slot*n_kv*D+kv_head*D+d);
            float sv=0.0f; int bvv=slot*rank*n_kv*D+kv_head*D+d;
            for(int r=0;r<rank;++r) sv+=wp[r] * READ_VAL(VV_pool, bvv+r*n_kv*D);
            val[idx]+=sv*bsc;
            ...
            int max_residual = args.max_residual;
            for (int r = 0; r < max_residual; ++r) {
                int t = res_V_pos[slot * max_residual + r];
                if (t >= 0 && t < slen) {
                    int base_res_v = slot * max_residual * n_kv * D + r * n_kv * D + kv_head * D;
                    res_v_sum += w_sh[t] * READ_VAL(res_V_val, base_res_v + d);
                }
            }
            val[idx]+=res_v_sum;
        }
    ...
    for (int t = split + runtime_nsplit*(int)tid; t < T_dense; t += runtime_nsplit*(int)ntg) {
        if (dense_mask[t] < -1e29f) { dw[t]=0.0f; continue; }
        int pos=dense_pos[t]; int bk=t*n_kv*D+kv_head*D;
        device const float4 * dense_K4 = (device const float4 *)(dense_K + bk);
        threadgroup const float4 * q_sh4 = (threadgroup const float4 *)q_sh;
        float sc = 0.0f;
        ...
        dw[t]=exp(sc*scale-ml);
    }
    threadgroup_barrier(mem_flags::mem_threadgroup);
    for(int d=(int)tid;d<D;d+=(int)ntg){ float acc=0.0f;
        for(int t=split; t<T_dense; t+=runtime_nsplit) acc+=dw[t]*dense_V[t*n_kv*D+kv_head*D+d];
        val[(d - (int)tid) / (int)ntg]+=acc; }
    if (split == 0) {
        int pos=cur_pos[0];
        int bk=kv_head*D;
        float sc=0.0f;
        ...
        float w=exp(sc*scale-ml);
        for(int d=(int)tid;d<D;d+=(int)ntg) val[(d - (int)tid) / (int)ntg]+=w*cur_v[kv_head*D+d];
    }
    if (runtime_nsplit == 1) {
        for (int d = (int)tid; d < D; d += (int)ntg) {
            scratch[head * D + d] = val[(d - (int)tid) / (int)ntg] / max(dl, 1e-9f);
        }
    } else {
        int sb = (head*DKV_NSPLIT + split)*(D+2);
        if (tid==0){ scratch[sb]=ml; scratch[sb+1]=dl; }
        for (int d=(int)tid; d<D; d+=(int)ntg) scratch[sb+2+d]=val[(d - (int)tid) / (int)ntg];
    }
    #undef ONLINE
    #undef READ_VAL
}

kernel void kernel_dkv_attn_combine(
        constant ggml_metal_kargs_dkv_attn & args [[buffer(0)]],
    ...
    const int D    = args.head_dim;
    float gm = -1e30f;
    for (int s=0; s<DKV_NSPLIT; ++s) gm = max(gm, scratch[(head*DKV_NSPLIT+s)*(D+2)]);
    float gd = 0.0f;
    for (int s=0; s<DKV_NSPLIT; ++s){ int b=(head*DKV_NSPLIT+s)*(D+2); gd += scratch[b+1]*exp(scratch[b]-gm); }
    gd = max(gd, 1e-9f);
    for (int d=(int)tid; d<D; d+=(int)ntg){
        float o=0.0f;
        for (int s=0; s<DKV_NSPLIT; ++s){ int b=(head*DKV_NSPLIT+s)*(D+2); o += scratch[b+2+d]*exp(scratch[b]-gm); }
        out[head*D+d] = o/gd;
    }
}
