...
#include "ggml-metal-impl.h"   // dkv_native: DKV_NSPLIT
...
static size_t ggml_backend_metal_buffer_type_get_alloc_size(ggml_backend_buffer_type_t buft, const ggml_tensor * tensor) {
    ...
    switch (tensor->op) {
        ...
        case GGML_OP_DKV_ATTN:
            {
                // dkv_native: flash-decoding per-split partials [n_q_heads * NSPLIT(16) * (D+2)]
                struct ggml_dkv_attn_params p;
                memcpy(&p, tensor->op_params, sizeof(p));
                res += (size_t) p.n_q_heads * DKV_NSPLIT * (p.head_dim + 2) * sizeof(float);
            } break;
        case GGML_OP_CUMSUM:
