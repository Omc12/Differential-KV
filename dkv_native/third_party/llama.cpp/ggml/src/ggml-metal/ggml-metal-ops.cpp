...
static int ggml_metal_op_encode_impl(ggml_metal_op_t ctx, int idx) {
    ...
    switch (node->op) {
        ...
        case GGML_OP_DKV_ATTN:
            {
                n_fuse = ggml_metal_op_dkv_attn(ctx, idx);
            } break;
        case GGML_OP_SET:
            {
...
// dkv_native: fused compressed-KV decode attention (one threadgroup per query head).
int ggml_metal_op_dkv_attn(ggml_metal_op_t ctx, int idx) {
    ggml_tensor * op = ctx->node(idx);
    ...
    ggml_metal_kargs_dkv_attn args = {
        /*.n_q_heads        =*/ p.n_q_heads,
        /*.n_kv_heads       =*/ p.n_kv_heads,
        /*.rank             =*/ p.rank,
        /*.s_max            =*/ p.s_max,
        /*.head_dim         =*/ p.head_dim,
        /*.n_blocks         =*/ p.n_blocks,
        /*.max_dense        =*/ p.max_dense,
        /*.t_dense          =*/ p.t_dense,
        /*.scale            =*/ p.scale,
        /*.has_rope         =*/ p.has_rope,
        /*.rope_freq_base   =*/ p.rope_freq_base,
        /*.approximate_attn =*/ p.approximate_attn,
        /*.n_slots          =*/ p.n_slots,
        /*.max_residual     =*/ p.max_residual,
        /*.is_q8            =*/ p.is_q8,
    };

    const int NSPLIT = (p.n_blocks <= 128) ? 1 : DKV_NSPLIT;
    const int nth = 64;      // threads/threadgroup (power of 2 for reductions)

    // scratch for per-split partials lives right after the output tensor in the same buffer
    // (sized by ggml_backend_metal_buffer_type_get_alloc_size): [n_q_heads * NSPLIT * (D+2)].
    ggml_metal_buffer_id bid_dst     = ggml_metal_get_buffer_id(op);
    ggml_metal_buffer_id bid_scratch = bid_dst;
    bid_scratch.offs += ggml_nbytes(op);

    // ── PARTIAL: head×split threadgroups → {m_local, d_local, O_local} per split ──
    auto pp = ggml_metal_library_compile_pipeline(lib, "kernel_dkv_attn_partial", "kernel_dkv_attn_partial", nullptr);
    ggml_metal_encoder_set_pipeline(enc, pp);
    ggml_metal_encoder_set_bytes   (enc, &args, sizeof(args), 0);
    for (int i = 0; i < 23; ++i) {
        ggml_metal_encoder_set_buffer(enc, ggml_metal_get_buffer_id(op->src[i]), i + 1);
    }
    if (NSPLIT == 1) {
        ggml_metal_encoder_set_buffer(enc, bid_dst, 24);
    } else {
        ggml_metal_encoder_set_buffer(enc, bid_scratch, 24);
    }
    ggml_metal_encoder_dispatch_threadgroups(enc, p.n_q_heads, NSPLIT, 1, nth, 1, 1);

    if (NSPLIT > 1) {
        ggml_metal_encoder_memory_barrier(enc); // combine must wait for all partials

        // ── COMBINE: merge the NSPLIT partials per head → output ──
        auto pc = ggml_metal_library_compile_pipeline(lib, "kernel_dkv_attn_combine", "kernel_dkv_attn_combine", nullptr);
        ggml_metal_encoder_set_pipeline(enc, pc);
        ggml_metal_encoder_set_bytes   (enc, &args, sizeof(args), 0);
        ggml_metal_encoder_set_buffer  (enc, bid_scratch, 1);
        ggml_metal_encoder_set_buffer  (enc, bid_dst,     2);
        ggml_metal_encoder_dispatch_threadgroups(enc, p.n_q_heads, 1, 1, nth, 1, 1);
    }
