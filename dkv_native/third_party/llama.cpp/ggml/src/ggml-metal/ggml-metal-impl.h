...
// dkv_native: flash-decoding split count (single source of truth for kernel + handler + alloc).
#define DKV_NSPLIT 8
// dkv_native: fused compressed-KV decode attention args (mirrors ggml_dkv_attn_params).
typedef struct {
    int32_t n_q_heads;
    int32_t n_kv_heads;
    int32_t rank;
