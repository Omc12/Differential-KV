...
// ggml_dkv_attn (dkv_native) — Metal-only fused compressed-KV decode attention.

struct ggml_tensor * ggml_dkv_attn(
        struct ggml_context * ctx,
        struct ggml_tensor  * q,
        struct ggml_tensor  * slot_indices,
        struct ggml_tensor  * u_pool,
        ...
        struct ggml_tensor  * dense_k,
        struct ggml_tensor  * dense_v,
        struct ggml_tensor  * dense_pos,
        struct ggml_tensor  * dense_mask,
        struct ggml_tensor  * cur_k,
        struct ggml_tensor  * cur_v,
        struct ggml_tensor  * cur_pos,
        struct ggml_tensor  * res_k_pos,
        struct ggml_tensor  * res_v_pos,
        struct ggml_tensor  * res_k_val,
        struct ggml_tensor  * res_v_val,
        struct ggml_tensor  * token_positions,
        struct ggml_dkv_attn_params params) {
    const int64_t n_embd = (int64_t) params.n_q_heads * params.head_dim;
    struct ggml_tensor * result = ggml_new_tensor_2d(ctx, GGML_TYPE_F32, n_embd, 1);

    ggml_set_op_params(result, &params, sizeof(params));

    result->op      = GGML_OP_DKV_ATTN;
    result->src[0]  = q;
    result->src[1]  = slot_indices;
    result->src[2]  = u_pool;
    result->src[3]  = u_scale;
    result->src[4]  = vk_pool;
    result->src[5]  = vv_pool;
    result->src[6]  = anchors_k;
    result->src[7]  = anchors_v;
    result->src[8]  = seq_lens;
    result->src[9]  = scales;
    result->src[10] = anchor_positions;
    result->src[11] = dense_k;
    result->src[12] = dense_v;
    result->src[13] = dense_pos;
    result->src[14] = dense_mask;
    result->src[15] = cur_k;
    result->src[16] = cur_v;
    result->src[17] = cur_pos;
    result->src[18] = res_k_pos;
    result->src[19] = res_v_pos;
    result->src[20] = res_k_val;
    result->src[21] = res_v_val;
    result->src[22] = token_positions;
