...
void ggml_backend_tensor_set(struct ggml_tensor * tensor, const void * data, size_t offset, size_t size) {
    ...
    if (offset + size > ggml_nbytes(tensor)) {
        fprintf(stderr, "[GGML_DEBUG_SET] tensor name: %s, offset: %zu, size: %zu, nbytes: %zu\n",
                tensor->name ? tensor->name : "(null)", offset, size, ggml_nbytes(tensor));
    }
    GGML_ASSERT(offset + size <= ggml_nbytes(tensor) && "tensor write out of bounds");
...
void ggml_backend_tensor_copy(const struct ggml_tensor * src, struct ggml_tensor * dst) {
    if (!ggml_are_same_layout(src, dst)) {
        fprintf(stderr, "[GGML_COPY_ERROR] src: %p, name='%s', type=%d, ne=[%lld,%lld,%lld,%lld], nb=[%zu,%zu,%zu,%zu]\n",
                (const void*)src, src->name, (int)src->type, (long long)src->ne[0], (long long)src->ne[1], (long long)src->ne[2], (long long)src->ne[3], src->nb[0], src->nb[1], src->nb[2], src->nb[3]);
        fprintf(stderr, "[GGML_COPY_ERROR] dst: %p, name='%s', type=%d, ne=[%lld,%lld,%lld,%lld], nb=[%zu,%zu,%zu,%zu]\n",
                (const void*)dst, dst->name, (int)dst->type, (long long)dst->ne[0], (long long)dst->ne[1], (long long)dst->ne[2], (long long)dst->ne[3], dst->nb[0], dst->nb[1], dst->nb[2], dst->nb[3]);
    }
