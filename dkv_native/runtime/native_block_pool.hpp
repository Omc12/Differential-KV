...
inline PoolRotMode pool_rot_mode() {
    static const PoolRotMode mode = []() {
        if (std::getenv("DKV_NO_ROTATE_AT_INGEST")) return POOL_ROT_RAW;
        if (const char* e = std::getenv("DKV_POOL_ABS_ROT")) {
...
public:
    ...
    // NO i32 position gather. nullptr unless DKV_NATIVE_ATTN is set.
    ...
    DKVBlockStateTable & get_state_table() { return state_table_; }
...
private:
    ...
    bool native_attn_ = false;        // set from DKV_NATIVE_ATTN in initialize()
