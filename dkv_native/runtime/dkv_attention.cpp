...
std::atomic<long> g_dkv_cb_invocations{0};
std::atomic<int>  g_dkv_dbg_pos{0};   // mirror of DKV_DBG_POS, set from main (worker-thread getenv fails)
std::atomic<int>  g_dkv_dbg_block_states{0}; // mirror of DKV_DBG_BLOCK_STATES, set from main
...
void cpu_dense_attention(
    ...
    if (std::getenv("DKV_DENSE_DIRECT")) consecutive = false; // force per-token cos/sin (no recurrence)
    ...
    for (int h = 0; h < n_q_heads; ++h) {
        ...
        if (std::getenv("DKV_DBG_KDENSE") && h < 7)
...
void custom_attention_op_callback(
    ...
    g_dkv_cb_invocations.fetch_add(1, std::memory_order_relaxed);  // unconditional: did the callback run?
    if (ith != 0) return;

    static const bool profile_cb = []() {
        const char* e = std::getenv("DKV_PROFILE_CB");
    ...
    static const bool route_all_resident = []() {
        const char* e = std::getenv("DKV_CB_ROUTE_ALL");
    ...
    int topk_blocks = 16;
    if (const char* env_topk = std::getenv("DKV_TOPK_BLOCKS")) {
    ...
    if (cache_active) {
        ...
        if (all_reused) {
            ...
            if (profile_cb) {
                std::printf("[DKV_PROFILE_CB] layer=%d actual_K=%d readback=%.3fms route=%.3fms gpu=%.3fms total=%.3fms (cached)\n",
    ...
    if (topk_blocks > 0 && actual_K > topk_blocks && data->kv_engine != nullptr) {
        ...
        static const bool gqa_route = []() {
            const char* e = std::getenv("DKV_CB_GQA_ROUTE");
            return !(e && std::string(e) == "0");
        }();
    ...
    if (c != nullptr && !data->ignore_c &&
        ...
        if (cap_ok) {
            ...
            if (!warned) {
                warned = true;
                std::cerr << "[DKV] WARNING: dense buffer full (T_dense=" << T_dense
...
#ifdef __APPLE__
    const char* env_cpu = std::getenv("DKV_FORCE_CPU_ATTN");
    ...
    if (!force_cpu) {
        ...
        double t_total_ms = std::chrono::duration<double, std::milli>(t_end - t_start).count();
        if (profile_cb) {
            std::printf("[DKV_PROFILE_CB] layer=%d actual_K=%d readback=%.3fms route=%.3fms gpu=%.3fms total=%.3fms\n",
                        data->layer_idx, actual_K, t_readback_ms, t_route_ms, t_gpu_ms, t_total_ms);
        }
        return;
    }
    ...
    if (std::getenv("DKV_DBG_INPUTS") && data->layer_idx == 0) {
        static int s = 0;
        if (s < 2) { s++;
    ...
    // DKV_DBG_LSE2: per-step layer-0 sparse-vs-dense LSE balance (D7 probe;
    // compare against MLX's DKV_DBG_LSE_SHARE on the same prompt/step).
    static const bool dbg_lse2 = (std::getenv("DKV_DBG_LSE2") != nullptr);
    ...
    static const bool dbg_sparseoff = (std::getenv("DKV_DBG_SPARSEOFF") != nullptr);
    static const bool dbg_denseoff  = (std::getenv("DKV_DBG_DENSEOFF2") != nullptr);
    ...
    if (std::getenv("DKV_DBG_ATTN0") && data->layer_idx == 0) {
