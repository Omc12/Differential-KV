...
struct AttentionParams {
    ...
    // Mirrors the struct in dkv_decode.metal — keep layouts in sync.
...
static void init_metal_runtime() {
    ...
    // 0. Explicit override via the DKV_METAL_DIR env var (portable; highest
    //    priority — lets a relocated binary point at the shader dir directly).
    if (const char* env_dir = std::getenv("DKV_METAL_DIR")) {
        path   = [NSString stringWithFormat:@"%s/dkv_decode.metal", env_dir];
    ...
    if (!source && _NSGetExecutablePath(exec_path, &sz) == 0) {
        ...
        path   = [binDir stringByAppendingPathComponent:@"../native_core/dkv_core/metal/dkv_decode.metal"];
        source = [NSString stringWithContentsOfFile:path encoding:NSUTF8StringEncoding error:&error];
        if (!source) {
            path   = [binDir stringByAppendingPathComponent:@"native_core/dkv_core/metal/dkv_decode.metal"];
            source = [NSString stringWithContentsOfFile:path encoding:NSUTF8StringEncoding error:&error];
    ...
    if (!source) {
        path   = @"../native_core/dkv_core/metal/dkv_decode.metal";
        source = [NSString stringWithContentsOfFile:path encoding:NSUTF8StringEncoding error:&error];
    }
    if (!source) {
        path   = @"native_core/dkv_core/metal/dkv_decode.metal";
    ...
    if (!source) {
        path   = @"dkv_native/native_core/dkv_core/metal/dkv_decode.metal";
        source = [NSString stringWithContentsOfFile:path encoding:NSUTF8StringEncoding error:&error];
    }

    // 3. Compile-time source-tree location — set by CMake to THIS build's source
    //    dir (see DKV_METAL_SOURCE_DIR in CMakeLists.txt). Correct on any build
    //    machine; not author-specific. Kept as a last-resort fallback after the
    //    env override + executable/CWD-relative probes above.
#ifdef DKV_METAL_SOURCE_DIR
    if (!source) {
        path   = @(DKV_METAL_SOURCE_DIR "/dkv_decode.metal");
        source = [NSString stringWithContentsOfFile:path encoding:NSUTF8StringEncoding error:&error];
    }
#endif

    if (!source) {
        std::cerr << "[Metal Attention] Error: Could not load dkv_decode.metal from any known location!" << std::endl;
        return;
    }
