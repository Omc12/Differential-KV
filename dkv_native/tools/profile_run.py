#!/usr/bin/env python3
"""
profile_run.py — Resource profiler for dkv_native
...
def main():
    ...
    env = os.environ.copy()
    env["DKV_NATIVE_ATTN"] = "1"
    env["DKV_MAX_TOKENS"]  = "60"

    print(f"{'='*60}")
    print(f"  dkv_native Resource Profiler  ({SAMPLE_SECS}s window)")
    print(f"{'='*60}")
    ...
    # ── Find the actual C++ dkv_native child process ───────────────────────
    # The Python CLI just spawns dkv_native as a subprocess and proxies I/O.
    # The real CPU/RAM is in the C++ child process, not the Python wrapper.
    target_pid = pid
    if HAS_PSUTIL:
        deadline_find = time.time() + 10
        print("[Profile] Waiting for C++ dkv_native child process...")
        while time.time() < deadline_find:
            try:
                children = psutil.Process(pid).children(recursive=True)
                for ch in children:
                    if "dkv_native" in ch.name() or "dkv_native" in " ".join(ch.cmdline()):
                        target_pid = ch.pid
