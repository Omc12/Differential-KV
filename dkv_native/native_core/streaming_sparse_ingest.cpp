...
extern std::atomic<int> g_dkv_dbg_pos;
extern std::atomic<int> g_dkv_dbg_block_states;
...
void StreamingSparseIngestManager::initialize(int n_layers, const DKVModel* model) {
    model_ = model;
    n_layers_ = n_layers;
...
// recency window unconditionally. Set DKV_SKIP_COMPRESSION_HEURISTIC=1 to restore
// it (factual/NIAH exact-retrieval path).
static bool skip_compression_heuristic_enabled() {
    static const bool enabled = []() {
        const char* e = std::getenv("DKV_SKIP_COMPRESSION_HEURISTIC");
...
void StreamingSparseIngestManager::ingest_chunk(
    ...
    for (int t = 0; t < chunk_len; ++t) {
        if (blocks.empty() || blocks.back()->token_count() == 1 + micro_block_size_) {
            ...
            if (slot_id != -1) {
                ...
                if (g_dkv_dbg_pos.load(std::memory_order_relaxed)) {
                    std::cerr << "[DBG_TRANS] Layer " << layer_idx << " anchor_idx=" << anchor_pos << " pool_idx=" << slot_id << " state=DenseResident\n";
                }
                if (g_dkv_dbg_block_states.load(std::memory_order_relaxed)) {
    ...
    if (const char* env_p = std::getenv("DKV_IMMEDIATE_PREFILL_COMPRESS")) {
    ...
    // independently re-reading DKV_ENGAGE_THRESHOLD here with a hardcoded
    ...
    bool bypass_dkv = ((int)token_ids.size() < engage_threshold);
    ...
    for (size_t idx = scan_start; idx < scan_end; ++idx) {
        ...
        if (b->state == BlockState::DenseResident && b->token_count() == 1 + micro_block_size_ && !b->skip_compression) {
            ...
            if (bypass_dkv) {
                ...
                //
                // ACTIVE_RUNTIME/mlx_dkv_wrapper.py compresses every block that falls
                ...
                // factual/NIAH exact-retrieval path via DKV_SKIP_COMPRESSION_HEURISTIC=1.
                //
...
void StreamingSparseIngestManager::submit_block_for_compression(
    ...
    // ── PER-TOKEN ROTATION AT INGEST (scheme selected by dkv::pool_rot_mode()) ──────────────
    // POOL_ROT_ABS (default): rotate each token at its ABSOLUTE sequence position
    //   (anchor_idx + t) — the same representation the MLX reference compresses. Decode
    //   then does NO rotation for pool content, which makes the cheap project-then-attend
    //   score path mathematically exact and removes a whole class of ingest/decode
    //   rotation-composition bugs (double rotation garbled all sparse output at HEAD).
    // POOL_ROT_WITHIN (DKV_POOL_ABS_ROT=0): legacy within-block-offset pre-rotation;
    //   only composes correctly with the approximate anchor-rotation decode paths.
    // POOL_ROT_RAW (DKV_NO_ROTATE_AT_INGEST): no pre-rotation; decode must rotate at
    //   true token positions.
    // K only; V is not RoPE'd. Applied BEFORE the landmark swap so the landmark carries
    // its own true position into the anchor slot.
    const PoolRotMode rot_mode = pool_rot_mode();
    if (rot_mode != POOL_ROT_RAW && engines[layer_idx]->get_has_rope()) {
        const int D = head_dim;
        const int nkv = (D > 0) ? (F_test / D) : 1;
        const int half_d = D / 2;
        const float freq = engines[layer_idx]->get_rope_freq_base();
        const int64_t base_pos = (rot_mode == POOL_ROT_ABS) ? (int64_t)block->anchor_idx : 0;
        for (int t = 0; t < S_total; ++t) {
            for (int kv = 0; kv < nkv; ++kv) {
                float* vec = block->svd_k.data() + (size_t)t * F_test + (size_t)kv * D;
                for (int d = 0; d < half_d; ++d) {
                    double theta = 1.0 / std::pow((double)freq, (2.0 * d) / D);
                    double angle = std::fmod((double)(base_pos + t) * theta, 2.0 * M_PI);
                    float cos_a = (float)std::cos(angle), sin_a = (float)std::sin(angle);
    ...
    if (boost) {
        ...
        if (layer_idx == 0) {
            std::cerr << "[DKV SVD Boost] Layer " << layer_idx << " Slot " << slot_id << " boosted to svd_rank=" << svd_rank << " (pool_rank=" << pool_rank << ")\n";
        }
    }
    ...
    bool async_svd = true;
    if (const char* env_async = std::getenv("DKV_ASYNC_SVD")) {
