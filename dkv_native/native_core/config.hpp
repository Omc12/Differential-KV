// dkv_native/include/config.hpp
// Translation of config.py → C++17
// DKVConfig: preset-based configuration with environment-variable overrides.
...
public:
        ...
        const std::unordered_map<std::string, std::string>& config_map = {})
    {
        ...
        preset = "mid"; // default
        {
            const char* env_preset = std::getenv("DKV_PRESET");
...
private:
    ...
    void apply_preset_defaults(const std::string& p) {
        if (p == "low") {
            ...
            if (p != "mid") {
                ...
                std::fprintf(stderr,
                    "[DKVConfig] Warning: unknown preset '%s', using 'mid'.\n",
                    p.c_str());
    ...
    void apply_env_overrides() {
        decode_cache_enabled =
            get_bool_env("DKV_DECODE_CACHE_ENABLED",  decode_cache_enabled);
        decode_cache_max_tokens =
            get_int_env ("DKV_DECODE_CACHE_MAX_TOKENS", decode_cache_max_tokens);
        prefill_chunk_size =
            get_int_env ("DKV_PREFILL_CHUNK_SIZE",    prefill_chunk_size);
        srl_threshold =
            get_int_env ("DKV_SRL_THRESHOLD",         srl_threshold);
        async_svd =
            get_bool_env("DKV_ASYNC_SVD",             async_svd);
        mps_watermark =
            get_float_env("PYTORCH_MPS_HIGH_WATERMARK_RATIO", mps_watermark);
        torch_compile =
            get_bool_env("DKV_USE_TORCH_COMPILE",     torch_compile);
        approximate_attn =
            get_bool_env("DKV_MPS_APPROXIMATE_ATTN",  approximate_attn);
        srl_age_penalty =
            get_float_env("DKV_SRL_AGE_PENALTY",      srl_age_penalty);
        early_layer_rank_boost =
            get_bool_env("DKV_EARLY_LAYER_RANK_BOOST", early_layer_rank_boost);
        max_rank_early =
            get_int_env ("DKV_MAX_RANK_EARLY",        max_rank_early);
    }
};
