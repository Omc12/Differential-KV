...
inline void process_and_tag_vsl_step(SessionSRLState& srl_state) {
    ...
    // 1. Contrastive Category Anchor
    {
        ...
        // dkv_attention.py:857 — only high-IDF tokens count for anchor overlap
    ...
    // 2. Coherence Cap — RC6 entity-proportional budget.
    {
        ...
        if ((int)srl_state.current_step_factual_sequences.size() > coherence_cap) {
            ...
            // dkv_attention.py:946-955 "DX1: triple sequences inherit their prime's score").
            for (const FactEntry* fe : srl_state.prime_entries) {
...
inline void update_vsl_state_cpp(
    ...
    if (std::getenv("DKV_VERBOSE")) {
        std::cerr << "[DKV VSL] Token: " << token_id 
    ...
    if (did_match_existing || did_start_new) {
            ...
            for (auto& entry : srl_state.factual_store.entries) {
                ...
                if (L >= 1 && tokens_match_normalized(entry.tokens[L - 1], token_id, model)) {
                    ...
                    if (match) {
                        entry.recalled = true;
                        if (std::getenv("DKV_VERBOSE")) {
                            std::cerr << "[DKV VSL] Completed lock and marked entry as recalled: token_id=" << token_id << ", length=" << L << "\n";
                        }
                            ...
                            for (auto& other : srl_state.factual_store.entries) {
                                ...
                                if (other.tokens.size() > entry.tokens.size()) {
                                    ...
                                    if (found) {
                                        other.recalled = true;
                                        if (std::getenv("DKV_VERBOSE")) {
                                            std::cerr << "[DKV VSL] Propagated recall to overlapping entry: length=" << other.tokens.size() << "\n";
                                        }
                                    }
                                }
                            }
        ...
        if (std::getenv("DKV_VERBOSE")) {
            std::cerr << "[DKV VSL] Committed lock: did_match_existing=" << did_match_existing 
    ...
    if (helper_ids.count(token_id) > 0) {
        ...
        if (std::getenv("DKV_VERBOSE")) {
            std::cerr << "[DKV VSL] Helper passthrough: consecutive=" << srl_state.vsl_consecutive_helpers << "\n";
        }
        if (srl_state.vsl_consecutive_helpers >= 12) {
            srl_state.vsl_active_candidates.clear();
            if (std::getenv("DKV_VERBOSE")) {
                std::cerr << "[DKV VSL] Helper threshold reached. Lock cleared.\n";
            }
        }
        return;
    }
