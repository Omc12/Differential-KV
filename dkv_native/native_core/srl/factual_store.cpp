...
void FactualExactStore::build(
    ...
    if (use_salience_parser) {
        ...
        if (L > 0) {
            ...
            std::cerr << "[DKV DIAG] Threshold val: " << threshold_val << "\n";
            for (int t = std::max(0, 2040); t < std::min(L, 2070); ++t) {
                std::cerr << "[DKV DIAG] Pos " << t << " token_id=" << token_ids[t]
...
std::vector<FactEntry> FactualExactStore::query(
    ...
    std::cerr << "[DKV Query DIAG] entries.size()=" << entries.size() 
    ...
    for (size_t idx = 0; idx < entries.size(); ++idx) {
        if (idx == 122 || idx == 123) {
            ...
            std::cerr << "[DKV Query DIAG] Entry " << idx << " | sim=" << sim << " | slots: ";
    ...
    for (int idx : all_candidate_idxs) {
        ...
        if (idx == 122 || idx == 123) {
            std::cerr << "[DKV Query DIAG] Entry " << idx << " | sim=" << sim 
    ...
    if (std::getenv("DKV_VERBOSE")) {
        std::cerr << "[DKV Query DIAG] merged_entries size=" << merged_entries.size() << "\n";
