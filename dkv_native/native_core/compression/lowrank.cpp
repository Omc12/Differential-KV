...
bool run_svd_driver(
    ...
    static const bool use_rand_svd = []() {
        if (const char* env = std::getenv("DKV_RAND_SVD")) {
            std::string s(env);
...
bool compress_lowrank_block(const LowRankCompressParams& params) {
    ...
    // into the stored VV basis (decode math unchanged). DKV_V_SCALE=0 disables.
    float v_gain = 1.0f;
    {
        static const bool v_scale_on = []() {
            const char* e = std::getenv("DKV_V_SCALE");
    ...
    *params.out_scale = ggml_fp32_to_fp16(scale);
    ...
    // ── DBG: reconstruction-error decomposition (DKV_DBG_COMPRESS_ERR=1) ─────
    ...
    if (std::getenv("DKV_DBG_COMPRESS_ERR")) {
    ...
    if (params.out_res_K_pos && params.out_res_V_pos &&
        ...
        if (std::getenv("DKV_DBG_RECON_ERR")) {
        ...
        int adaptive_MR = MR;
        // DKV_RESIDUAL_UNIFORM=1 (set by the native LEGO prefill, studs far
        ...
        const char* res_uni_env = std::getenv("DKV_RESIDUAL_UNIFORM");
        ...
        static const float cov_frac = []() {
            const char* e = std::getenv("DKV_RESIDUAL_COVERAGE_FRAC");
        ...
        // ids 48-57, which are NOT digits in this vocab). DKV_RESIDUAL_TOKEN_BOOST=0
        ...
        if (params.token_ids) {
            static const float tok_boost = []() {
                const char* e = std::getenv("DKV_RESIDUAL_TOKEN_BOOST");
                if (e) { try { return std::stof(e); } catch (...) {} }
                return 8.0f;
            }();
            if (tok_boost > 1.0f) {
                ...
                // _apply_owner_capture, DKV_RESIDUAL_OWNER_CAPTURE default ON).
                ...
                // name left), and boost those rows with the same idf weight.
                {
                    static const bool owner_on = []() {
                        const char* e = std::getenv("DKV_RESIDUAL_OWNER_CAPTURE");
                        return !(e && std::string(e) == "0");
                    }();
                    static const int owner_dist = []() {
                        const char* e = std::getenv("DKV_RESIDUAL_OWNER_DIST");
                ...
                // _apply_table_capture, DKV_RESIDUAL_TABLE_CAPTURE).
                ...
                // tokens) gets the core boost × DKV_RESIDUAL_TABLE_PRIORITY
                ...
                // (separator dashes, pipes) degrade first under saturation.
                {
                    static const bool table_on = []() {
                        ...
                        const char* e = std::getenv("DKV_RESIDUAL_TABLE_CAPTURE");
                        return !(e && std::string(e) == "0");
                    }();
                    static const float table_priority = []() {
                        const char* e = std::getenv("DKV_RESIDUAL_TABLE_PRIORITY");
                        if (e) { try { return std::stof(e); } catch (...) {} }
                        return 4.0f;
                    }();
                    if (table_on) {
                        ...
                        // DKV_RESIDUAL_TABLE_CAPTION=0 disables.
                        static const bool caption_on = []() {
                            const char* e = std::getenv("DKV_RESIDUAL_TABLE_CAPTION");
                            return !(e && std::string(e) == "0");
                        }();
                        if (caption_on) {
                            auto lower_c = [](char c) {
        ...
        // Margin env: DKV_RESIDUAL_FLOOR_MARGIN (default 4).
        static const int floor_margin = []() {
            const char* e = std::getenv("DKV_RESIDUAL_FLOOR_MARGIN");
            if (e) { try { return std::stoi(e); } catch (...) {} }
            return 4;
        }();
        ...
        static const bool dbg_residuals = (std::getenv("DKV_DBG_RESIDUALS") != nullptr);
