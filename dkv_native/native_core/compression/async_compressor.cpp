...
AsyncCompressor::AsyncCompressor(DKVBlockStateTable& state_table, std::function<bool(const std::string&)> alive_cb)
...
bool AsyncCompressor::start() {
    ...
    // without starving the decode. Override with DKV_COMPRESSOR_THREADS.
    int num_threads = std::max(2, std::min(6, (int)(std::thread::hardware_concurrency() / 2)));
    if (const char* env_nt = std::getenv("DKV_COMPRESSOR_THREADS")) {
...
void AsyncCompressor::process_job(const CompressJob& job) {
    DKVBlockStateTable& active_table = job.state_table ? *job.state_table : state_table_;
    ...
    if (trans_ok) {
        ...
        if (g_dkv_dbg_block_states.load(std::memory_order_relaxed)) {
            std::cerr << "[DBG_BLOCK_STATES] Session " << job.session_id << " block_id=" << job.local_block_id << " anchor_idx=" << job.anchor_idx << " pool_idx=" << job.block_id << " state=CompressedResident\n";
        }
    } else {
        active_table.force_invalidate(job.block_id);
        jobs_dropped_.fetch_add(1, std::memory_order_relaxed);
