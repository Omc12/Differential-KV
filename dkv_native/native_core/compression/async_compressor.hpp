...
struct CompressJob {
    ...
    int layer_idx = -1;  // diagnostic-only: for DKV_DBG_RECON_ERR per-layer breakdown
    ...
    int max_residual = 8;
    DKVBlockStateTable* state_table = nullptr;
...
public:
    AsyncCompressor(DKVBlockStateTable& state_table, std::function<bool(const std::string&)> alive_cb = nullptr);
    ~AsyncCompressor();
