// dkv_core/include/compressor_cpu.hpp
...
//       DKVBlockStateTable confirms CompressedResident state.
...
#include <atomic>
#include <functional>
#include "native_core/dkv_core/include/spsc_ring_buffer.hpp"
...
// ── DKVCompressorThreadCPU ─────────────────────────────────────────────────
// Drop-in replacement for DKVCompressorThread on Mac.
// Uses Accelerate LAPACK (LAPACKE_sgesdd) for truncated SVD.
// Zero Metal/MPS interaction.
class DKVCompressorThreadCPU {
public:
    static constexpr size_t QUEUE_CAPACITY = 4096; // Power of 2

    explicit DKVCompressorThreadCPU(
        DKVBlockStateTable& state_table,
