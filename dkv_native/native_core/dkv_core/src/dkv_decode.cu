...
#define DKV_CUDA_CHECK(call) do {                                              \
    cudaError_t _e = (call);                                                      \
    if (_e != cudaSuccess) {                                                      \
        std::cerr << "[DKV CUDA] " << #call << " failed: "                     \
...
// ── FULL DKV CUDA Attention Kernel (F10) ─────────────────────────────────
// Mirrors execute_cpu_attention() (runtime/dkv_attention.cpp), approximate_attn
...
__global__ void dkv_full_decode_kernel(
...
void execute_cuda_attention(
    ...
    if (d_split_out == nullptr || d_split_units < needed_units || d_split_D != D) {
        ...
        DKV_CUDA_CHECK(cudaMalloc(&d_split_out, (size_t)needed_units * D * sizeof(float)));
        DKV_CUDA_CHECK(cudaMalloc(&d_split_m,   (size_t)needed_units * sizeof(float)));
        DKV_CUDA_CHECK(cudaMalloc(&d_split_d,   (size_t)needed_units * sizeof(float)));
    ...
    if (K > 0 && slot_indices != nullptr) {
        if (d_slot_indices_scratch == nullptr || scratch_capacity < K) {
            ...
            DKV_CUDA_CHECK(cudaMalloc(&d_slot_indices_scratch, std::max(256, K) * sizeof(int32_t)));
            scratch_capacity = std::max(256, K);
        }
        DKV_CUDA_CHECK(cudaMemcpy(d_slot_indices_scratch, slot_indices, K * sizeof(int32_t), cudaMemcpyHostToDevice));
    }
    ...
    // F10: full DKV kernel (anchor + low-rank deltas + exact residuals + dense),
    // one thread per output dim. Legacy anchor+dense stub via DKV_CUDA_ANCHOR_ONLY=1
    // for A/B. Correctness fallback is the CPU path (DKV_FORCE_CPU_ATTN=1).
    const char* anchor_only_env = std::getenv("DKV_CUDA_ANCHOR_ONLY");
    if (!(anchor_only_env && std::string(anchor_only_env) == "1")) {
        dim3 block(D, 1, 1);
        size_t shared_size = ((size_t)D + rank + S_max + rank + 32) * sizeof(float);
        dkv_full_decode_kernel<<<grid, block, shared_size>>>(
            d_Q,
        ...
        DKV_CUDA_CHECK(cudaGetLastError());
    } else {
        dim3 block(64, 1, 1);
    ...
    DKV_CUDA_CHECK(cudaStreamSynchronize(0));
}
