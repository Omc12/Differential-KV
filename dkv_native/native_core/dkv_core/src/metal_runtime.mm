...
#include "native_core/dkv_core/src/dkv_metallib.hpp"
#include <iostream>
#include <vector>
#include <cmath>
#include <algorithm>
...
public:
    ...
    MetalDecodePipeline() {
        @autoreleasepool {
            ...
            if (!device) {
                std::cerr << "[DKV Metal] Failed to retrieve system default MTLDevice!" << std::endl;
                return;
            }
            queue = [device newCommandQueue];
            if (!queue) {
                std::cerr << "[DKV Metal] Failed to create MTLCommandQueue!" << std::endl;
                return;
            }
