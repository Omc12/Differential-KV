...
int KVRuntimeManager::get_layer_rank(int layer_idx) const {
    ...
    // (DKV_DBG_RECON_ERR, native_core/compression/lowrank.cpp) run on a
    ...
    if (ratio < 0.15) {
        ...
        if (const char* boost_env = std::getenv("DKV_EARLY_LAYER_RANK_BOOST")) {
...
std::vector<int32_t> KVRuntimeManager::route_decode_slots(
    ...
    if (n_ord <= 36) {
        ...
        if (const char* env = std::getenv("DKV_SRL_DECAY_FACTOR")) {
        ...
        if (const char* dbg = std::getenv("DKV_DBG_SEMROUTE")) {
        ...
        if (!srl_state.recent_decode_keys.empty() && !engines_.empty()) {
            ...
            if (std::getenv("DKV_DBG_SEMROUTE")) {
    ...
    // ACTIVE_RUNTIME/runtime/dkv_attention.py:544:
...
void KVRuntimeManager::sync_device_for_native() {
    ...
    static const bool sync_all = std::getenv("DKV_SYNC_ALL") != nullptr;
    if (sync_all) {
    ...
    for (int l = 0; l < n_layers_; ++l) {
        ...
        for (auto & block : ingest_manager_->get_blocks(l)) {
            if (block->pool_idx != -1 && block->pool_idx < ns) {
                ...
                // them → glitches that DKV_SYNC_ALL masked. Include all non-empty states.
                if (st != BlockState::Freed && st != BlockState::Invalid) {
