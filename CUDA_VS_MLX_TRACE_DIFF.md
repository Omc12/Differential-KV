...
behavioural difference between `serving/mlx_diffkv_wrapper.py` (MLX, the working
reference) and the CUDA path (`serving/hf_diffkv_wrapper.py` → `runtime/diffkv_attention.py`
...
| CAD (relational accuracy) | high/quality/max → `DIFFKV_CAD_ALPHA=0.5`, `CAD_MAX_STEPS=32` | **was never auto-set** → now aligned (hf_diffkv_wrapper) | 🟢 *(fixed)* |
| allocator/cache cap | `DIFFKV_CACHE_LIMIT_GB=1` (`mx.set_cache_limit`) | `PYTORCH_CUDA_ALLOC_CONF` (expandable_segments via eval) | 🟡 different mechanism |
...
| **V-scale** | `sqrt(eK/eV)` on V before joint K\|V SVD, unscale factor after | **was absent** → now ported (`DIFFKV_V_SCALE=1`), V recon err 0.97→0.56 | 🟢 *(fixed)* |
...
| | MLX | CUDA | |
|---|---|---|---|
| engagement | `DIFFKV_COMPRESSED_DECODE=1` → **sparse from token 1** (accepts 16 vs 36 tps @4k, "always exercise DiffKV") | dense **bypass below `ENGAGE_THRESHOLD=4096`** | 🟡 CUDA is *faster* short-ctx — deliberate |
...
| decode cache var | `DIFFKV_DECODE_CACHE` (works) | ⚫ **`DIFFKV_DECODE_CACHE` is read NOWHERE on CUDA** (only in `decode_config.py`'s defaults dict). CUDA's real one is `DIFFKV_DECODE_CACHE_ENABLED` → the gathered-KV workspace cache in the *PyTorch fallback*, off for accuracy (Issue-6 stale tensors) | 🔴 the "~2× tps default" is a **no-op on CUDA** |
...
3. 🔴 **Routing threshold** (§7) — CUDA attends all 49 blocks at 13k vs MLX's 16. One-env-var A/B (`DIFFKV_SRL_THRESHOLD=16`); accuracy-gated.
...
6. 🔴 **`DIFFKV_DECODE_CACHE` is a no-op on CUDA** (§7) — the documented "2× tps" default does nothing here.
</content>
